...
시작하기 전에, Guzzle 패키지가 애플리케이션의 종속성으로 설치되어 있는지 확인해야 합니다. 기본적으로 Laravel은 이 종속성을 자동으로 포함합니다. 그러나 이전에 패키지를 제거한 경우, Composer를 통해 다시 설치할 수 있습니다:
...
다행히 `pool` 메서드를 사용하여 이를 달성할 수 있습니다. `pool` 메서드는 `Illuminate\Http\Client\Pool` 인스턴스를 받는 클로저를 받아서 요청 풀에 요청을 쉽게 추가하여 발송할 수 있습니다.
