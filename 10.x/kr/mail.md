...
이메일 발송은 복잡할 필요가 없습니다. Laravel은 인기 있는 [Symfony Mailer](https://symfony.com/doc/6.2/mailer.html) 컴포넌트를 기반으로 하는 깔끔하고 간단한 이메일 API를 제공합니다. Laravel과 Symfony Mailer는 SMTP, Mailgun, Postmark, Amazon SES, 그리고 `sendmail`을 통해 이메일을 발송할 수 있는 드라이버를 제공하여, 로컬 또는 클라우드 기반 서비스를 통해 빠르게 메일 발송을 시작할 수 있습니다.
...
Mailgun, Postmark, MailerSend와 같은 API 기반 드라이버는 SMTP 서버를 통한 메일 발송보다 더 간단하고 빠른 경우가 많습니다. 가능하다면 이러한 드라이버 중 하나를 사용하는 것을 권장합니다.
...
다음으로, 애플리케이션의 `config/mail.php` 설정 파일에서 `default` 옵션을 `mailgun`으로 설정합니다. 애플리케이션의 기본 메일러를 설정한 후, `config/services.php` 설정 파일에 다음 옵션이 포함되어 있는지 확인하세요:
...
트랜잭션 이메일 및 SMS 서비스인 [MailerSend](https://www.mailersend.com/)는 Laravel용 자체 API 기반 메일 드라이버를 유지 관리합니다. 드라이버를 포함하는 패키지는 Composer 패키지 관리자를 통해 설치할 수 있습니다:

```shell
composer require mailersend/laravel-driver
```

패키지가 설치되면, 애플리케이션의 `.env` 파일에 `MAILERSEND_API_KEY` 환경 변수를 추가합니다. 또한, `MAIL_MAILER` 환경 변수를 `mailersend`로 정의해야 합니다:
...
호스팅된 템플릿 사용 방법을 포함한 MailerSend에 대한 자세한 내용은 [MailerSend 드라이버 문서](https://github.com/mailersend/mailersend-laravel-driver#usage)를 참조하세요.
...
Laravel은 메일 메시지를 발송하는 동안 두 개의 이벤트를 디스패치합니다. `MessageSending` 이벤트는 메시지가 발송되기 전에 디스패치되고, `MessageSent` 이벤트는 메시지가 발송된 후에 디스패치됩니다. 이러한 이벤트는 메일이 큐에 들어갈 때가 아니라 *발송*될 때 디스패치된다는 것을 기억하세요. `App\Providers\EventServiceProvider` 서비스 프로바이더에서 이러한 이벤트에 대한 이벤트 리스너를 등록할 수 있습니다:
