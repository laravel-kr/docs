...
Laravel Tinker는 [PsySH](https://github.com/bobthecow/psysh) 패키지로 구동되는 Laravel 프레임워크를 위한 강력한 REPL입니다.
...
Tinker는 셸 내에서 실행할 수 있는 아티즌 명령어를 결정하기 위해 "허용" 목록을 사용합니다. 기본적으로 `clear-compiled`, `down`, `env`, `inspire`, `migrate`, `optimize`, `up` 명령어를 실행할 수 있습니다. 더 많은 명령어를 허용하려면 `tinker.php` 설정 파일의 `commands` 배열에 추가하면 됩니다:
...
명령어를 생성한 후 클래스의 `signature` 및 `description` 속성에 적절한 값을 정의해야 합니다. 이러한 속성은 `list` 화면에서 명령어를 표시할 때 사용됩니다. `signature` 속성을 사용하면 [명령어의 입력 기대값](#defining-input-expectations)을 정의할 수도 있습니다. `handle` 메서드는 명령어가 실행될 때 호출됩니다. 이 메서드에 명령어 로직을 배치할 수 있습니다.
...
명령어가 `Isolatable`로 표시되면 Laravel은 자동으로 `--isolated` 옵션을 명령어에 추가합니다. 해당 옵션으로 명령어가 호출되면 Laravel은 해당 명령어의 다른 인스턴스가 이미 실행 중이지 않은지 확인합니다. Laravel은 애플리케이션의 기본 캐시 드라이버를 사용하여 원자적 잠금을 획득하려고 시도하여 이를 달성합니다. 명령어의 다른 인스턴스가 실행 중인 경우 명령어가 실행되지 않습니다. 그러나 명령어는 여전히 성공적인 종료 상태 코드로 종료됩니다:
...
모든 콘솔 명령어는 애플리케이션의 "콘솔 커널"인 `App\Console\Kernel` 클래스 내에서 등록됩니다. 이 클래스의 `commands` 메서드 내에서 커널의 `load` 메서드 호출을 볼 수 있습니다. `load` 메서드는 `app/Console/Commands` 디렉토리를 스캔하고 해당 디렉토리에 포함된 각 명령어를 아티즌에 자동으로 등록합니다. 다른 디렉토리를 스캔하기 위해 `load` 메서드를 추가로 호출할 수도 있습니다:
...
필요한 경우 `App\Console\Kernel` 클래스 내의 `$commands` 속성에 명령어의 클래스 이름을 추가하여 명령어를 수동으로 등록할 수 있습니다. 이 속성이 커널에 아직 정의되어 있지 않다면 수동으로 정의해야 합니다. 아티즌이 부팅되면 이 속성에 나열된 모든 명령어가 [서비스 컨테이너(Service Container)](/docs/{{version}}/container)에 의해 해결되고 아티즌에 등록됩니다:
