...
Laravel의 기본 Tailwind 페이지네이션 뷰와 Tailwind JIT 엔진을 사용하는 경우, Tailwind 클래스가 제거되지 않도록 애플리케이션의 `tailwind.config.js` 파일의 `content` 키가 Laravel의 페이지네이션 뷰를 참조하도록 해야 합니다.
