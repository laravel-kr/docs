...
`unique` 메서드는 컬렉션에서 모든 고유한 모델을 반환합니다. 컬렉션의 다른 모델과 동일한 타입이면서 동일한 기본 키를 가진 모델은 제거됩니다.
...
특정 모델과 상호작용할 때 커스텀 `Collection` 객체를 사용하려면 모델에 `newCollection` 메서드를 정의할 수 있습니다.
...
`newCollection` 메서드를 정의하면, Eloquent가 일반적으로 `Illuminate\Database\Eloquent\Collection` 인스턴스를 반환하는 모든 경우에 커스텀 컬렉션의 인스턴스를 받게 됩니다. 애플리케이션의 모든 모델에 대해 커스텀 컬렉션을 사용하려면, 애플리케이션의 모든 모델이 확장하는 기본 모델 클래스에 `newCollection` 메서드를 정의해야 합니다.
