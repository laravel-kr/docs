...
먼저, [새 Laravel 애플리케이션을 생성](/docs/{{version}}/installation)하고, 데이터베이스를 설정한 후 [데이터베이스 마이그레이션](/docs/{{version}}/migrations)을 실행해야 합니다. 새 Laravel 애플리케이션을 생성한 후 Composer를 사용하여 Laravel Breeze를 설치할 수 있습니다.

```shell
composer require laravel/breeze --dev
```

Composer가 Laravel Breeze 패키지를 설치한 후, `breeze:install` 아티즌 명령어를 실행할 수 있습니다. 이 명령어는 인증 뷰, 라우트, 컨트롤러 및 기타 리소스를 애플리케이션에 퍼블리시합니다. Laravel Breeze는 모든 코드를 애플리케이션에 퍼블리시하므로 기능과 구현에 대한 완전한 제어와 가시성을 갖게 됩니다.
