...
- [JavaScript를 사용한 API 소비](#consuming-your-api-with-javascript)
- [이벤트](#events)
- [테스팅](#testing)
...
Passport의 [서비스 프로바이더](/docs/{{version}}/providers)는 자체 데이터베이스 마이그레이션 디렉토리를 등록하므로, 패키지를 설치한 후 데이터베이스를 마이그레이션해야 합니다. Passport 마이그레이션은 애플리케이션이 OAuth2 클라이언트와 액세스 토큰을 저장하는 데 필요한 테이블을 생성합니다:

```shell
php artisan migrate
```

다음으로, `passport:install` Artisan 명령어를 실행해야 합니다. 이 명령어는 보안 액세스 토큰을 생성하는 데 필요한 암호화 키를 생성합니다. 또한 이 명령어는 액세스 토큰을 생성하는 데 사용될 "개인용 액세스" 및 "패스워드 그랜트" 클라이언트를 생성합니다:
...
> [!NOTE]
> Passport `Client` 모델의 기본 키 값으로 자동 증가 정수 대신 UUID를 사용하려면 [`uuids` 옵션](#client-uuids)을 사용하여 Passport를 설치하세요.

`passport:install` 명령어를 실행한 후, `App\Models\User` 모델에 `Laravel\Passport\HasApiTokens` 트레이트를 추가하세요. 이 트레이트는 인증된 사용자의 토큰과 스코프를 검사할 수 있는 몇 가지 헬퍼 메서드를 모델에 제공합니다. 모델이 이미 `Laravel\Sanctum\HasApiTokens` 트레이트를 사용하고 있다면 해당 트레이트를 제거할 수 있습니다:
...
`passport:install` 명령어에 `--uuids` 옵션을 사용하여 실행할 수도 있습니다. 이 옵션은 Passport의 `Client` 모델의 기본 키 값으로 자동 증가 정수 대신 UUID를 사용하도록 지시합니다. `--uuids` 옵션으로 `passport:install` 명령어를 실행한 후, Passport의 기본 마이그레이션 비활성화에 대한 추가 지침이 제공됩니다:
...
필요한 경우, Passport의 키를 로드할 경로를 정의할 수 있습니다. 이를 위해 `Passport::loadKeysFrom` 메서드를 사용할 수 있습니다. 일반적으로 이 메서드는 애플리케이션의 `App\Providers\AuthServiceProvider` 클래스의 `boot` 메서드에서 호출해야 합니다.
...
Passport의 기본 마이그레이션을 사용하지 않으려면, `App\Providers\AppServiceProvider` 클래스의 `register` 메서드에서 `Passport::ignoreMigrations` 메서드를 호출해야 합니다. `vendor:publish` Artisan 명령어를 사용하여 기본 마이그레이션을 내보낼 수 있습니다:
...
클라이언트의 시크릿이 데이터베이스에 저장될 때 해싱되기를 원한다면, `App\Providers\AuthServiceProvider` 클래스의 `boot` 메서드에서 `Passport::hashClientSecrets` 메서드를 호출해야 합니다.

    use Laravel\Passport\Passport;

    Passport::hashClientSecrets();

활성화되면, 모든 클라이언트 시크릿은 생성된 직후에만 사용자에게 표시됩니다. 평문 클라이언트 시크릿 값은 데이터베이스에 절대 저장되지 않으므로, 분실된 시크릿 값을 복구하는 것은 불가능합니다.

<a name="token-lifetimes"></a>
### 토큰 수명

기본적으로 Passport는 1년 후에 만료되는 장기 액세스 토큰을 발급합니다. 더 길거나 짧은 토큰 수명을 설정하려면 `tokensExpireIn`, `refreshTokensExpireIn`, `personalAccessTokensExpireIn` 메서드를 사용할 수 있습니다. 이러한 메서드는 애플리케이션의 `App\Providers\AuthServiceProvider` 클래스의 `boot` 메서드에서 호출해야 합니다.
...
모델을 정의한 후, `Laravel\Passport\Passport` 클래스를 통해 Passport에게 커스텀 모델을 사용하도록 지시할 수 있습니다. 일반적으로 애플리케이션의 `App\Providers\AuthServiceProvider` 클래스의 `boot` 메서드에서 Passport에게 커스텀 모델에 대해 알려야 합니다.
...
그런 다음, [라우트 파일](https://github.com/laravel/passport/blob/11.x/routes/web.php)에서 Passport가 정의한 라우트를 애플리케이션의 `routes/web.php` 파일에 복사하고 원하는 대로 수정할 수 있습니다.
...
먼저, 애플리케이션의 API와 상호 작용해야 하는 애플리케이션을 개발하는 개발자들은 "클라이언트"를 생성하여 애플리케이션을 여러분의 애플리케이션에 등록해야 합니다. 일반적으로 이것은 애플리케이션의 이름과 사용자가 인가 요청을 승인한 후 리다이렉트할 수 있는 URL을 제공하는 것으로 구성됩니다.

<a name="the-passportclient-command"></a>
#### `passport:client` 명령어

클라이언트를 생성하는 가장 간단한 방법은 `passport:client` Artisan 명령어를 사용하는 것입니다. 이 명령어는 OAuth2 기능을 테스트하기 위해 자체 클라이언트를 생성하는 데 사용할 수 있습니다. `client` 명령어를 실행하면 Passport가 클라이언트에 대한 추가 정보를 요청하고 클라이언트 ID와 시크릿을 제공합니다.
...
클라이언트에 대해 여러 리다이렉트 URL을 허용하려면 `passport:client` 명령어에서 URL을 요청할 때 쉼표로 구분된 목록을 사용하여 지정할 수 있습니다. 쉼표가 포함된 URL은 URL 인코딩되어야 합니다.
...
애플리케이션의 사용자는 `client` 명령어를 사용할 수 없으므로, Passport는 클라이언트를 생성하는 데 사용할 수 있는 JSON API를 제공합니다. 이를 통해 클라이언트를 생성, 업데이트, 삭제하기 위한 컨트롤러를 수동으로 코딩하는 번거로움을 줄일 수 있습니다.

그러나 사용자가 클라이언트를 관리할 수 있는 대시보드를 제공하기 위해 Passport의 JSON API를 자체 프론트엔드와 결합해야 합니다. 아래에서 클라이언트 관리를 위한 모든 API 엔드포인트를 검토합니다. 편의를 위해 엔드포인트에 대한 HTTP 요청을 보여주기 위해 [Axios](https://github.com/axios/axios)를 사용합니다.

JSON API는 `web`과 `auth` 미들웨어로 보호됩니다. 따라서 자체 애플리케이션에서만 호출할 수 있습니다. 외부 소스에서 호출할 수 없습니다.

<a name="get-oauthclients"></a>
#### `GET /oauth/clients`

이 라우트는 인증된 사용자의 모든 클라이언트를 반환합니다. 이는 사용자가 클라이언트를 편집하거나 삭제할 수 있도록 모든 클라이언트를 나열하는 데 주로 유용합니다.
...
이 라우트는 새 클라이언트를 생성하는 데 사용됩니다. 클라이언트의 `name`과 `redirect` URL 두 가지 데이터가 필요합니다. `redirect` URL은 사용자가 인가 요청을 승인하거나 거부한 후 리다이렉트되는 곳입니다.

클라이언트가 생성되면, 클라이언트 ID와 클라이언트 시크릿이 발급됩니다. 이 값들은 애플리케이션에서 액세스 토큰을 요청할 때 사용됩니다. 클라이언트 생성 라우트는 새 클라이언트 인스턴스를 반환합니다.
...
이 라우트는 클라이언트를 업데이트하는 데 사용됩니다. 클라이언트의 `name`과 `redirect` URL 두 가지 데이터가 필요합니다. `redirect` URL은 사용자가 인가 요청을 승인하거나 거부한 후 리다이렉트되는 곳입니다. 라우트는 업데이트된 클라이언트 인스턴스를 반환합니다.
...
인가 승인 화면을 커스터마이징하려면, `vendor:publish` Artisan 명령어를 사용하여 Passport의 뷰를 발행할 수 있습니다. 발행된 뷰는 `resources/views/vendor/passport` 디렉토리에 배치됩니다.
...
Passport에는 인가된 액세스 토큰을 관리하기 위한 JSON API도 포함되어 있습니다. 이를 자체 프론트엔드와 결합하여 사용자에게 액세스 토큰을 관리하기 위한 대시보드를 제공할 수 있습니다. 편의를 위해 엔드포인트에 대한 HTTP 요청을 보여주기 위해 [Axios](https://github.com/mzabriskie/axios)를 사용합니다. JSON API는 `web`과 `auth` 미들웨어로 보호됩니다. 따라서 자체 애플리케이션에서만 호출할 수 있습니다.

<a name="get-oauthtokens"></a>
#### `GET /oauth/tokens`

이 라우트는 인증된 사용자가 생성한 모든 인가된 액세스 토큰을 반환합니다. 이는 사용자가 토큰을 폐기할 수 있도록 모든 토큰을 나열하는 데 주로 유용합니다.
...
이 라우트는 인가된 액세스 토큰과 관련 리프레시 토큰을 폐기하는 데 사용됩니다.
...
`Laravel\Passport\TokenRepository`의 `revokeAccessToken` 메서드를 사용하여 토큰을 폐기할 수 있습니다. `Laravel\Passport\RefreshTokenRepository`의 `revokeRefreshTokensByAccessTokenId` 메서드를 사용하여 토큰의 리프레시 토큰을 폐기할 수 있습니다. 이 클래스들은 Laravel의 [서비스 컨테이너](/docs/{{version}}/container)를 사용하여 리졸브할 수 있습니다.
...
애플리케이션의 `App\Console\Kernel` 클래스에서 [스케줄 작업](/docs/{{version}}/scheduling)을 설정하여 일정에 따라 토큰을 자동으로 정리할 수도 있습니다.
...
"코드 교환을 위한 증명 키"(PKCE)를 사용하는 인가 코드 그랜트는 단일 페이지 애플리케이션이나 네이티브 애플리케이션이 API에 액세스하기 위해 인증하는 안전한 방법입니다. 이 그랜트는 클라이언트 시크릿이 기밀로 저장될 것이라고 보장할 수 없거나, 공격자에 의해 인가 코드가 가로채지는 위협을 완화하기 위해 사용해야 합니다. 인가 코드를 액세스 토큰으로 교환할 때 "코드 검증자"와 "코드 챌린지"의 조합이 클라이언트 시크릿을 대체합니다.
...
애플리케이션이 패스워드 그랜트를 통해 토큰을 발급하기 전에, 패스워드 그랜트 클라이언트를 생성해야 합니다. `passport:client` Artisan 명령어에 `--password` 옵션을 사용하여 이를 수행할 수 있습니다. **`passport:install` 명령어를 이미 실행했다면 이 명령어를 실행할 필요가 없습니다.**
...
패스워드 그랜트 클라이언트를 생성하면, 사용자의 이메일 주소와 비밀번호로 `/oauth/token` 라우트에 `POST` 요청을 발행하여 액세스 토큰을 요청할 수 있습니다. 이 라우트는 이미 Passport에 의해 등록되어 있으므로 수동으로 정의할 필요가 없습니다. 요청이 성공하면 서버로부터 JSON 응답으로 `access_token`과 `refresh_token`을 받게 됩니다.
...
애플리케이션이 둘 이상의 [인증 사용자 프로바이더](/docs/{{version}}/authentication#introduction)를 사용하는 경우, `artisan passport:client --password` 명령어를 통해 클라이언트를 생성할 때 `--provider` 옵션을 제공하여 패스워드 그랜트 클라이언트가 사용하는 사용자 프로바이더를 지정할 수 있습니다. 지정된 프로바이더 이름은 애플리케이션의 `config/auth.php` 설정 파일에 정의된 유효한 프로바이더와 일치해야 합니다. 그런 다음 [미들웨어를 사용하여 라우트를 보호](#via-middleware)하여 가드에 지정된 프로바이더의 사용자만 인가되도록 할 수 있습니다.
...
암시적 그랜트는 인가 코드 그랜트와 유사하지만, 인가 코드를 교환하지 않고 토큰이 클라이언트에 반환됩니다. 이 그랜트는 클라이언트 자격증명이 안전하게 저장될 수 없는 JavaScript 또는 모바일 애플리케이션에 가장 일반적으로 사용됩니다. 그랜트를 활성화하려면 애플리케이션의 `App\Providers\AuthServiceProvider` 클래스의 `boot` 메서드에서 `enableImplicitGrant` 메서드를 호출하세요.
...
그랜트가 활성화되면, 개발자는 클라이언트 ID를 사용하여 애플리케이션에서 액세스 토큰을 요청할 수 있습니다. 사용하는 애플리케이션은 다음과 같이 애플리케이션의 `/oauth/authorize` 라우트로 리다이렉트 요청을 해야 합니다.
...
다음으로, 이 그랜트 타입을 사용하려면 `CheckClientCredentials` 미들웨어를 애플리케이션의 `app/Http/Kernel.php` 파일의 `$middlewareAliases` 속성에 추가하세요.
...
특정 스코프로 라우트에 대한 액세스를 제한하려면, `client` 미들웨어를 라우트에 연결할 때 필요한 스코프의 쉼표로 구분된 목록을 제공할 수 있습니다.
...
애플리케이션이 개인용 액세스 토큰을 발급하기 전에, 개인용 액세스 클라이언트를 생성해야 합니다. `passport:client` Artisan 명령어에 `--personal` 옵션을 사용하여 이를 수행할 수 있습니다. `passport:install` 명령어를 이미 실행했다면 이 명령어를 실행할 필요가 없습니다.

```shell
php artisan passport:client --personal
```

개인용 액세스 클라이언트를 생성한 후, 클라이언트의 ID와 평문 시크릿 값을 애플리케이션의 `.env` 파일에 넣으세요.
...
Passport에는 개인용 액세스 토큰을 관리하기 위한 JSON API도 포함되어 있습니다. 이를 자체 프론트엔드와 결합하여 사용자에게 개인용 액세스 토큰을 관리하기 위한 대시보드를 제공할 수 있습니다. 아래에서 개인용 액세스 토큰을 관리하기 위한 모든 API 엔드포인트를 검토합니다. 편의를 위해 엔드포인트에 대한 HTTP 요청을 보여주기 위해 [Axios](https://github.com/mzabriskie/axios)를 사용합니다.
...
이 라우트는 애플리케이션에 정의된 모든 [스코프](#token-scopes)를 반환합니다. 이 라우트를 사용하여 사용자가 개인용 액세스 토큰에 할당할 수 있는 스코프를 나열할 수 있습니다.
...
이 라우트는 인증된 사용자가 생성한 모든 개인용 액세스 토큰을 반환합니다. 이는 사용자가 토큰을 편집하거나 폐기할 수 있도록 모든 토큰을 나열하는 데 주로 유용합니다.
...
이 라우트는 새 개인용 액세스 토큰을 생성합니다. 토큰의 `name`과 토큰에 할당해야 하는 `scopes` 두 가지 데이터가 필요합니다.
...
이 라우트는 개인용 액세스 토큰을 폐기하는 데 사용됩니다.
...
> [!WARNING]
> [클라이언트 자격증명 그랜트](#client-credentials-grant-tokens)를 사용하는 경우, `auth:api` 미들웨어 대신 [`client` 미들웨어](#client-credentials-grant-tokens)를 사용하여 라우트를 보호해야 합니다.
...
> [!NOTE]
> Passport와 함께 여러 사용자 프로바이더를 사용하는 방법에 대한 자세한 내용은 [패스워드 그랜트 문서](#customizing-the-user-provider)를 참조하세요.

<a name="passing-the-access-token"></a>
### 액세스 토큰 전달하기

Passport로 보호된 라우트를 호출할 때, 애플리케이션의 API 소비자는 요청의 `Authorization` 헤더에 `Bearer` 토큰으로 액세스 토큰을 지정해야 합니다. 예를 들어, Guzzle HTTP 라이브러리를 사용할 때:
...
애플리케이션의 `App\Providers\AuthServiceProvider` 클래스의 `boot` 메서드에서 `Passport::tokensCan` 메서드를 사용하여 API의 스코프를 정의할 수 있습니다. `tokensCan` 메서드는 스코프 이름과 스코프 설명의 배열을 받습니다. 스코프 설명은 원하는 대로 작성할 수 있으며 인가 승인 화면에서 사용자에게 표시됩니다.
...
클라이언트가 특정 스코프를 요청하지 않으면, `setDefaultScope` 메서드를 사용하여 Passport 서버가 토큰에 기본 스코프를 첨부하도록 설정할 수 있습니다. 일반적으로 이 메서드는 애플리케이션의 `App\Providers\AuthServiceProvider` 클래스의 `boot` 메서드에서 호출해야 합니다.
...
> [!NOTE]
> Passport의 기본 스코프는 사용자가 생성한 개인용 액세스 토큰에는 적용되지 않습니다.
...
Passport에는 들어오는 요청이 주어진 스코프가 부여된 토큰으로 인증되었는지 확인하는 데 사용할 수 있는 두 개의 미들웨어가 포함되어 있습니다. 시작하려면 `app/Http/Kernel.php` 파일의 `$middlewareAliases` 속성에 다음 미들웨어를 추가하세요.
...
`scopes` 미들웨어는 들어오는 요청의 액세스 토큰이 나열된 모든 스코프를 가지고 있는지 확인하기 위해 라우트에 할당될 수 있습니다.

    Route::get('/orders', function () {
        // 액세스 토큰이 "check-status"와 "place-orders" 스코프를 모두 가지고 있습니다...
    })->middleware(['auth:api', 'scopes:check-status,place-orders']);

<a name="check-for-any-scopes"></a>
#### 스코프 중 하나 확인

`scope` 미들웨어는 들어오는 요청의 액세스 토큰이 나열된 스코프 중 *적어도 하나*를 가지고 있는지 확인하기 위해 라우트에 할당될 수 있습니다.

    Route::get('/orders', function () {
        // 액세스 토큰이 "check-status" 또는 "place-orders" 스코프를 가지고 있습니다...
...
일반적으로 JavaScript 애플리케이션에서 API를 사용하려면 액세스 토큰을 애플리케이션에 수동으로 전송하고 각 요청과 함께 전달해야 합니다. 그러나 Passport에는 이를 처리할 수 있는 미들웨어가 포함되어 있습니다. `app/Http/Kernel.php` 파일에서 `web` 미들웨어 그룹에 `CreateFreshApiToken` 미들웨어를 추가하기만 하면 됩니다.
...
필요한 경우 `Passport::cookie` 메서드를 사용하여 `laravel_token` 쿠키의 이름을 커스터마이징할 수 있습니다. 일반적으로 이 메서드는 애플리케이션의 `App\Providers\AuthServiceProvider` 클래스의 `boot` 메서드에서 호출해야 합니다.
...
이 인증 방법을 사용할 때 요청에 유효한 CSRF 토큰 헤더가 포함되어 있는지 확인해야 합니다. 기본 Laravel JavaScript 스캐폴딩에는 Axios 인스턴스가 포함되어 있으며, 이 인스턴스는 동일 출처 요청에서 암호화된 `XSRF-TOKEN` 쿠키 값을 사용하여 `X-XSRF-TOKEN` 헤더를 자동으로 전송합니다.
...
Passport는 액세스 토큰과 리프레시 토큰을 발급할 때 이벤트를 발생시킵니다. 데이터베이스에서 다른 액세스 토큰을 정리하거나 폐기하기 위해 [이러한 이벤트를 수신](/docs/{{version}}/events)할 수 있습니다. 원하는 경우 애플리케이션의 `App\Providers\EventServiceProvider` 클래스에서 이러한 이벤트에 리스너를 등록할 수 있습니다.
