...
> [!NOTE]
> 컨트롤러가 반드시 기본 클래스를 상속해야 하는 것은 **아닙니다**. 하지만 기본 클래스를 상속하지 않으면 `middleware`나 `authorize` 메서드와 같은 편리한 기능에 접근할 수 없습니다.
...
또는 컨트롤러의 생성자 내에서 미들웨어를 지정하는 것이 편리할 수 있습니다. 컨트롤러의 생성자 내에서 `middleware` 메서드를 사용하여 컨트롤러의 액션에 미들웨어를 할당할 수 있습니다.
...
컨트롤러에서 클로저를 사용하여 미들웨어를 등록할 수도 있습니다. 이는 전체 미들웨어 클래스를 정의하지 않고 단일 컨트롤러를 위한 인라인 미들웨어를 정의하는 편리한 방법을 제공합니다:
...
기본적으로 `Route::resource`는 영어 동사와 복수형 규칙을 사용하여 리소스 URI를 생성합니다. `create` 및 `edit` 액션 동사를 지역화해야 하는 경우, `Route::resourceVerbs` 메서드를 사용할 수 있습니다. 이는 애플리케이션의 `App\Providers\RouteServiceProvider` 내 `boot` 메서드 시작 부분에서 수행할 수 있습니다.
...
이 예제에서 `photos` 리소스는 모든 [표준 리소스 라우트](#actions-handled-by-resource-controller)를 받습니다. 그러나 `thumbnail` 리소스는 다음 라우트를 가진 싱글톤 리소스가 됩니다.
