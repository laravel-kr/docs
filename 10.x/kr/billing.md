...
> [!WARNING]
> [단건 결제](#single-charges) 또는 [단건 결제 체크아웃](#single-charge-checkouts)에는 세금이 계산되지 않습니다.
...
[종량제 청구](https://stripe.com/docs/billing/subscriptions/metered-billing)를 사용하면 청구 주기 동안 고객의 제품 사용량에 따라 요금을 부과할 수 있습니다. 예를 들어, 고객이 월별로 보내는 문자 메시지나 이메일 수에 따라 요금을 부과할 수 있습니다.

종량제 청구를 시작하려면 먼저 Stripe 대시보드에서 종량제 가격으로 새 제품을 생성해야 합니다. 그런 다음 `meteredPrice`를 사용하여 고객 구독에 종량제 가격 ID를 추가합니다:
...
고객이 애플리케이션을 사용할 때 정확한 청구를 위해 사용량을 Stripe에 보고해야 합니다. 종량제 구독의 사용량을 증가시키려면 `reportUsage` 메서드를 사용할 수 있습니다:
...
애플리케이션이 단일 구독에 여러 가격을 제공하는 경우, `reportUsageFor` 메서드를 사용하여 사용량을 보고할 종량제 가격을 지정해야 합니다:
...
때로는 이전에 보고한 사용량을 업데이트해야 할 수 있습니다. 이를 위해 `reportUsage`의 두 번째 매개변수로 타임스탬프 또는 `DateTimeInterface` 인스턴스를 전달할 수 있습니다. 이렇게 하면 Stripe는 해당 시간에 보고된 사용량을 업데이트합니다. 지정된 날짜와 시간이 현재 청구 기간 내에 있는 한 이전 사용량 기록을 계속 업데이트할 수 있습니다:
...
애플리케이션이 단일 구독에 여러 가격을 제공하는 경우, `usageRecordsFor` 메서드를 사용하여 사용량 기록을 조회할 종량제 가격을 지정할 수 있습니다:
...
`usageRecords` 및 `usageRecordsFor` 메서드는 사용량 기록의 연관 배열을 포함하는 Collection 인스턴스를 반환합니다. 이 배열을 반복하여 고객의 총 사용량을 표시할 수 있습니다:
...
반환되는 모든 사용량 데이터와 Stripe의 커서 기반 페이지네이션 사용 방법에 대한 전체 참조는 [공식 Stripe API 문서](https://stripe.com/docs/api/usage_records/subscription_item_summary_list)를 참고하세요.
...
Stripe 웹훅은 Laravel의 [CSRF 보호](/docs/{{version}}/csrf)를 우회해야 하므로, 애플리케이션의 `App\Http\Middleware\VerifyCsrfToken` 미들웨어에서 해당 URI를 예외로 등록하거나, 라우트를 `web` 미들웨어 그룹 외부에 배치해야 합니다:
...
리스너가 정의되면 애플리케이션의 `EventServiceProvider`에서 등록할 수 있습니다:
...
게스트 체크아웃이 완료된 후, Stripe는 `checkout.session.completed` 웹훅 이벤트를 전송할 수 있으므로, 이 이벤트를 실제로 애플리케이션에 보내도록 [Stripe 웹훅을 구성](https://dashboard.stripe.com/webhooks)해야 합니다. Stripe 대시보드에서 웹훅이 활성화되면 [Cashier로 웹훅을 처리](#handling-stripe-webhooks)할 수 있습니다. 웹훅 페이로드에 포함된 객체는 고객의 주문을 이행하기 위해 검사할 수 있는 [`checkout` 객체](https://stripe.com/docs/api/checkout/sessions/object)입니다.
...
Cashier를 사용하는 애플리케이션을 테스트할 때 Stripe API에 대한 실제 HTTP 요청을 모킹할 수 있지만, 이 경우 Cashier의 자체 동작을 부분적으로 다시 구현해야 합니다. 따라서 테스트가 실제 Stripe API를 호출하도록 허용하는 것을 권장합니다. 이 방식이 더 느리지만 애플리케이션이 예상대로 작동하는지에 대한 더 높은 신뢰도를 제공하며, 느린 테스트는 자체 PHPUnit 테스트 그룹에 배치할 수 있습니다.
