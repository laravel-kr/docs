...
일부 JSON API 표준에서는 리소스 및 리소스 컬렉션 응답에 메타 데이터를 추가해야 합니다. 이는 종종 리소스 또는 관련 리소스에 대한 `links`나 리소스 자체에 대한 메타 데이터와 같은 것을 포함합니다. 리소스에 대한 추가 메타 데이터를 반환해야 하는 경우 `toArray` 메서드에 포함하세요. 예를 들어, 리소스 컬렉션을 변환할 때 `link` 정보를 포함할 수 있습니다.
