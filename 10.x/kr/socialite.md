...
Laravel은 일반적인 폼 기반 인증 외에도, [Laravel Socialite](https://github.com/laravel/socialite)를 사용하여 OAuth 제공자를 통해 간단하고 편리하게 인증할 수 있는 방법을 제공합니다. Socialite는 현재 Facebook, Twitter, LinkedIn, Google, GitHub, GitLab, Bitbucket, Slack을 통한 인증을 지원합니다.
...
이러한 자격 증명은 애플리케이션의 `config/services.php` 설정 파일에 배치해야 하며, 애플리케이션에서 필요로 하는 제공자에 따라 `facebook`, `twitter` (OAuth 1.0), `twitter-oauth-2` (OAuth 2.0), `linkedin-openid`, `google`, `github`, `gitlab`, `bitbucket`, 또는 `slack` 키를 사용해야 합니다.
...
사용자에 대한 유효한 토큰과 시크릿이 이미 있는 경우, Socialite의 `userFromTokenAndSecret` 메서드를 사용하여 사용자 정보를 조회할 수 있습니다.
