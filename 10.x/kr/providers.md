...
Laravel에 포함된 `config/app.php` 파일을 열면 `providers` 배열을 볼 수 있습니다. 이 배열에는 애플리케이션에 로드될 모든 서비스 프로바이더 클래스가 나열되어 있습니다. 기본적으로 이 배열에는 메일러, 큐, 캐시 등 핵심 Laravel 컴포넌트를 부트스트랩하는 Laravel 핵심 서비스 프로바이더 세트가 나열되어 있습니다. 이러한 프로바이더 중 상당수는 "지연(deferred)" 프로바이더입니다. 즉, 모든 요청에서 로드되지 않고 실제로 해당 서비스가 필요할 때만 로드됩니다.

이 개요에서는 자신만의 서비스 프로바이더를 작성하고 Laravel 애플리케이션에 등록하는 방법을 배우게 됩니다.
...
Artisan CLI를 사용하여 `make:provider` 명령으로 새 프로바이더를 생성할 수 있습니다.
...
모든 서비스 프로바이더는 `config/app.php` 설정 파일에 등록됩니다. 이 파일에는 서비스 프로바이더의 클래스 이름을 나열할 수 있는 `providers` 배열이 포함되어 있습니다. 기본적으로 이 배열에는 메일러, 큐, 캐시 등 핵심 Laravel 컴포넌트를 부트스트랩하는 Laravel 핵심 서비스 프로바이더 세트가 등록되어 있습니다.

프로바이더를 등록하려면 배열에 추가합니다:
