...
이 설정 파일들을 통해 데이터베이스 연결 정보, 메일 서버 정보, 그리고 애플리케이션 시간대(timezone)와 암호화 키와 같은 다양한 핵심 설정 값들을 구성할 수 있습니다.

<a name="application-overview"></a>
#### 애플리케이션 개요

급하시다면 `about` Artisan 명령어를 통해 애플리케이션의 설정, 드라이버, 환경에 대한 빠른 개요를 확인할 수 있습니다.
...
Laravel의 기본 `.env` 파일에는 애플리케이션이 로컬에서 실행되는지 프로덕션 웹 서버에서 실행되는지에 따라 달라질 수 있는 몇 가지 공통 설정 값이 포함되어 있습니다. 이 값들은 `config` 디렉토리 내의 다양한 Laravel 설정 파일에서 Laravel의 `env` 함수를 사용하여 조회됩니다.

팀과 함께 개발하는 경우, 애플리케이션과 함께 `.env.example` 파일을 계속 포함하는 것이 좋습니다. 예제 설정 파일에 플레이스홀더 값을 넣어두면, 팀의 다른 개발자들이 애플리케이션을 실행하는 데 어떤 환경 변수가 필요한지 명확하게 알 수 있습니다.
