...
애플리케이션의 세션 설정 파일은 `config/session.php`에 저장되어 있습니다. 이 파일에서 사용 가능한 옵션들을 검토해 보세요. 기본적으로 Laravel은 `file` 세션 드라이버를 사용하도록 설정되어 있으며, 이는 많은 애플리케이션에서 잘 작동합니다. 애플리케이션이 여러 웹 서버 간에 로드 밸런싱되는 경우 모든 서버가 접근할 수 있는 중앙 저장소(예: Redis 또는 데이터베이스)를 선택해야 합니다.

세션 `driver` 설정 옵션은 각 요청에 대해 세션 데이터가 저장될 위치를 정의합니다. Laravel에는 여러 훌륭한 드라이버가 기본 제공됩니다.
...
`database` 세션 드라이버를 사용할 때는 세션 레코드를 포함할 테이블을 생성해야 합니다. 테이블에 대한 `Schema` 선언 예제는 아래에서 확인할 수 있습니다:
...
`session:table` Artisan 명령어를 사용하여 이 마이그레이션을 생성할 수 있습니다. 데이터베이스 마이그레이션에 대해 더 자세히 알아보려면 전체 [마이그레이션 문서](/docs/{{version}}/migrations)를 참조하세요:
...
> [!NOTE]  
> `session` 설정 파일에서 `connection` 옵션을 사용하여 세션에 사용할 Redis 연결을 지정할 수 있습니다.
...
> [!WARNING]  
> 세션 블로킹을 활용하려면 애플리케이션이 [원자적 잠금(Atomic Locks)](/docs/{{version}}/cache#atomic-locks)을 지원하는 캐시 드라이버를 사용해야 합니다. 현재 해당 캐시 드라이버에는 `memcached`, `dynamodb`, `redis`, `database`, `file`, `array` 드라이버가 포함됩니다. 또한 `cookie` 세션 드라이버는 사용할 수 없습니다.
...
> [!NOTE]
> Laravel에는 확장을 보관할 디렉토리가 기본 제공되지 않습니다. 원하는 곳에 배치할 수 있습니다. 이 예제에서는 `MongoSessionHandler`를 보관할 `Extensions` 디렉토리를 만들었습니다.

이러한 메서드의 목적은 쉽게 이해할 수 없으므로 각 메서드가 하는 일을 빠르게 살펴보겠습니다.
