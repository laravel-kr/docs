...
애플리케이션의 기본 언어는 `config/app.php` 설정 파일의 `locale` 설정 옵션에 저장됩니다. 애플리케이션의 필요에 맞게 이 값을 자유롭게 수정할 수 있습니다.
...
활성 언어에 주어진 번역 문자열이 포함되지 않은 경우 사용될 "대체 언어(fallback language)"를 설정할 수 있습니다. 기본 언어와 마찬가지로 대체 언어도 `config/app.php` 설정 파일에서 설정됩니다.
