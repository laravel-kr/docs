...
라라벨의 기본 큐 시스템과 달리, Horizon은 세 가지 워커 밸런싱 전략 중에서 선택할 수 있습니다: `simple`, `auto`, `false`. `simple` 전략은 들어오는 작업을 워커 프로세스 간에 균등하게 분배합니다:

    'balance' => 'simple',

`auto` 전략은 설정 파일의 기본값으로, 큐의 현재 작업량에 따라 큐별 워커 프로세스 수를 조정합니다. 예를 들어, `notifications` 큐에 1,000개의 대기 중인 작업이 있고 `render` 큐가 비어 있으면, Horizon은 큐가 비어질 때까지 `notifications` 큐에 더 많은 워커를 할당합니다.

`auto` 전략을 사용할 때, 큐별 최소 프로세스 수와 Horizon이 스케일 업 및 다운해야 하는 최대 총 워커 프로세스 수를 제어하기 위해 `minProcesses`와 `maxProcesses` 설정 옵션을 정의할 수 있습니다:
...
`autoScalingStrategy` 설정 값은 Horizon이 큐를 처리하는 데 걸리는 총 시간(`time` 전략) 또는 큐에 있는 총 작업 수(`size` 전략)를 기반으로 더 많은 워커 프로세스를 큐에 할당할지 결정합니다.

`balanceMaxShift`와 `balanceCooldown` 설정 값은 Horizon이 워커 수요를 충족하기 위해 얼마나 빠르게 스케일링할지 결정합니다. 위의 예에서는 3초마다 최대 하나의 새 프로세스가 생성되거나 제거됩니다. 애플리케이션의 필요에 따라 이 값을 자유롭게 조정할 수 있습니다.

`balance` 옵션이 `false`로 설정되면 기본 라라벨 동작이 사용되며, 설정에 나열된 순서대로 큐가 처리됩니다.
...
> [!NOTE]  
> Supervisor를 직접 설정하는 것이 부담스럽다면, 라라벨 프로젝트에 대해 Supervisor를 자동으로 설치하고 설정해주는 [Laravel Forge](https://forge.laravel.com) 사용을 고려해 보세요.
