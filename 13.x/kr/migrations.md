...
`migrate` 명령에 `--step` 옵션을 제공하면, 각 마이그레이션이 개별 배치로 실행되어 나중에 `migrate:rollback` 명령을 사용하여 개별 마이그레이션을 롤백할 수 있습니다:
...
PostgreSQL을 사용하는 경우, `vector` 컬럼을 생성하기 전에 `pgvector` 확장이 로드되어 있어야 합니다:
