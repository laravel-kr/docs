...
[Laravel MCP](https://github.com/laravel/mcp)는 AI 클라이언트가 [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro)을 통해 여러분의 Laravel 애플리케이션과 상호작용할 수 있는 간단하고 우아한 방법을 제공합니다. 서버, 도구(Tool), 리소스(Resource), 프롬프트(Prompt)를 정의할 수 있는 표현력 있고 유창한 인터페이스를 제공하여 AI 기반의 애플리케이션 상호작용을 가능하게 합니다.

<a name="installation"></a>
## 설치

시작하려면 Composer 패키지 매니저를 사용하여 프로젝트에 Laravel MCP를 설치하세요.
...
Laravel MCP를 설치한 후, `vendor:publish` Artisan 명령어를 실행하여 MCP 서버를 정의할 `routes/ai.php` 파일을 퍼블리싱하세요.
...
`make:mcp-server` Artisan 명령어를 사용하여 MCP 서버를 생성할 수 있습니다. 서버는 도구, 리소스, 프롬프트와 같은 MCP 기능을 AI 클라이언트에 노출하는 중앙 통신 지점 역할을 합니다.

```shell
php artisan make:mcp-server WeatherServer
```

이 명령어는 `app/Mcp/Servers` 디렉터리에 새로운 서버 클래스를 생성합니다. 생성된 서버 클래스는 Laravel MCP의 기본 `Laravel\Mcp\Server` 클래스를 확장하며, 서버를 설정하고 도구, 리소스, 프롬프트를 등록하기 위한 어트리뷰트와 속성을 제공합니다:
...
서버를 생성한 후에는 `routes/ai.php` 파일에 서버를 등록하여 접근 가능하도록 해야 합니다. Laravel MCP는 서버를 등록하기 위한 두 가지 방법을 제공합니다. HTTP로 접근 가능한 서버를 위한 `web`과 커맨드라인 서버를 위한 `local`입니다.

<a name="web-servers"></a>
### 웹 서버

웹 서버는 가장 일반적인 유형의 서버로, HTTP POST 요청을 통해 접근할 수 있어 원격 AI 클라이언트나 웹 기반 통합에 이상적입니다. `web` 메서드를 사용하여 웹 서버를 등록하세요.
...
일반 라우트와 마찬가지로 미들웨어(Middleware)를 적용하여 웹 서버를 보호할 수 있습니다.
...
로컬 서버는 Artisan 명령어로 실행되며, [Laravel Boost](/docs/{{version}}/installation#installing-laravel-boost)와 같은 로컬 AI 어시스턴트 통합을 구축하는 데 적합합니다. `local` 메서드를 사용하여 로컬 서버를 등록하세요.
...
등록이 완료되면 일반적으로 `mcp:start` Artisan 명령어를 직접 수동으로 실행할 필요가 없습니다. 대신 MCP 클라이언트(AI 에이전트)가 서버를 시작하도록 구성하거나 [MCP Inspector](#mcp-inspector)를 사용하세요.

<a name="tools"></a>
## 도구

도구(Tool)는 AI 클라이언트가 호출할 수 있는 기능을 서버에서 노출할 수 있게 해줍니다. 언어 모델이 작업을 수행하거나, 코드를 실행하거나, 외부 시스템과 상호작용할 수 있도록 합니다.
...
도구를 생성하려면 `make:mcp-tool` Artisan 명령어를 실행하세요.

```shell
php artisan make:mcp-tool CurrentWeatherTool
```

도구를 생성한 후, 서버의 `$tools` 속성에 등록하세요.
...
도구 설명은 자동으로 생성되지 않습니다. 항상 `Description` 어트리뷰트를 사용하여 의미 있는 설명을 제공해야 합니다:
...
> [!NOTE]
> 설명은 도구의 메타데이터에서 중요한 부분으로, AI 모델이 도구를 언제 어떻게 효과적으로 사용할 수 있는지 이해하는 데 도움을 줍니다.

<a name="tool-input-schemas"></a>
### 도구 입력 스키마

도구는 AI 클라이언트로부터 받는 인수를 지정하기 위해 입력 스키마를 정의할 수 있습니다. Laravel의 `Illuminate\Contracts\JsonSchema\JsonSchema` 빌더를 사용하여 도구의 입력 요구사항을 정의하세요.
...
도구는 응답 구조를 지정하기 위해 [출력 스키마](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#output-schema)를 정의할 수 있습니다. 이를 통해 파싱 가능한 도구 결과가 필요한 AI 클라이언트와의 더 나은 통합이 가능합니다. `outputSchema` 메서드를 사용하여 도구의 출력 구조를 정의하세요.
...
JSON 스키마 정의는 도구 인수의 기본 구조를 제공하지만, 더 복잡한 유효성 검사 규칙을 적용하고 싶을 수도 있습니다.

Laravel MCP는 Laravel의 [유효성 검사 기능](/docs/{{version}}/validation)과 원활하게 통합됩니다. 도구의 `handle` 메서드 내에서 수신된 도구 인수를 검증할 수 있습니다.
...
유효성 검사 실패 시, AI 클라이언트는 여러분이 제공하는 오류 메시지에 따라 행동합니다. 따라서 명확하고 실행 가능한 오류 메시지를 제공하는 것이 중요합니다.
...
Laravel [서비스 컨테이너](/docs/{{version}}/container)가 모든 도구를 해석하는 데 사용됩니다. 따라서 도구의 생성자에서 필요한 의존성을 타입-힌트할 수 있습니다. 선언된 의존성은 자동으로 해석되어 도구 인스턴스에 주입됩니다.
...
생성자 주입 외에도, 도구의 `handle()` 메서드에서 의존성을 타입-힌트할 수도 있습니다. 메서드가 호출될 때 서비스 컨테이너가 자동으로 의존성을 해석하고 주입합니다.
...
[어노테이션](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations)을 사용하여 AI 클라이언트에 추가 메타데이터를 제공함으로써 도구를 향상시킬 수 있습니다. 이러한 어노테이션은 AI 모델이 도구의 동작과 기능을 이해하는 데 도움을 줍니다. 어노테이션은 어트리뷰트를 통해 도구에 추가됩니다.
...
도구 클래스에서 `shouldRegister` 메서드를 구현하여 런타임에 조건부로 도구를 등록할 수 있습니다. 이 메서드를 통해 애플리케이션 상태, 설정 또는 요청 파라미터에 따라 도구의 사용 가능 여부를 결정할 수 있습니다.
...
도구의 `shouldRegister` 메서드가 `false`를 반환하면 사용 가능한 도구 목록에 나타나지 않으며 AI 클라이언트에 의해 호출될 수 없습니다.

<a name="tool-responses"></a>
### 도구 응답

도구는 `Laravel\Mcp\Response` 인스턴스를 반환해야 합니다. Response 클래스는 다양한 유형의 응답을 생성하기 위한 여러 편리한 메서드를 제공합니다.
...
`Response` 인스턴스의 배열을 반환하여 여러 개의 콘텐츠를 반환할 수 있습니다.
...
도구는 `structured` 메서드를 사용하여 [구조화된 콘텐츠](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#structured-content)를 반환할 수 있습니다. 이는 JSON 인코딩된 텍스트 표현과의 하위 호환성을 유지하면서 AI 클라이언트에 파싱 가능한 데이터를 제공합니다.
...
구조화된 콘텐츠와 함께 커스텀 텍스트를 제공해야 하는 경우, 응답 팩토리의 `withStructuredContent` 메서드를 사용하세요.
...
장시간 실행되는 작업이나 실시간 데이터 스트리밍의 경우, 도구는 `handle` 메서드에서 [제너레이터](https://www.php.net/manual/en/language.generators.overview.php)를 반환할 수 있습니다. 이를 통해 최종 응답 전에 클라이언트에 중간 업데이트를 보낼 수 있습니다.
...
웹 기반 서버를 사용할 때, 스트리밍 응답은 자동으로 SSE(Server-Sent Events) 스트림을 열고 각 yield된 메시지를 이벤트로 클라이언트에 전송합니다.

<a name="prompts"></a>
## 프롬프트

[프롬프트](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts)는 AI 클라이언트가 언어 모델과 상호작용하는 데 사용할 수 있는 재사용 가능한 프롬프트 템플릿을 서버에서 공유할 수 있게 해줍니다. 일반적인 쿼리와 상호작용을 구조화하는 표준화된 방법을 제공합니다.
...
기본적으로 프롬프트의 이름과 제목은 클래스 이름에서 자동으로 생성됩니다. 예를 들어, `DescribeWeatherPrompt`의 이름은 `describe-weather`이고 제목은 `Describe Weather Prompt`이 됩니다. `Name`과 `Title` 어트리뷰트를 사용하여 이 값들을 커스터마이즈할 수 있습니다:
...
프롬프트 설명은 자동으로 생성되지 않습니다. 항상 `Description` 어트리뷰트를 사용하여 의미 있는 설명을 제공해야 합니다:
...
> [!NOTE]
> 설명은 프롬프트의 메타데이터에서 중요한 부분으로, AI 모델이 프롬프트를 언제 어떻게 최대한 활용할 수 있는지 이해하는 데 도움을 줍니다.

<a name="prompt-arguments"></a>
### 프롬프트 인수

프롬프트는 AI 클라이언트가 특정 값으로 프롬프트 템플릿을 커스터마이즈할 수 있도록 인수를 정의할 수 있습니다. `arguments` 메서드를 사용하여 프롬프트가 받는 인수를 정의하세요.
...
프롬프트 인수는 정의에 따라 자동으로 유효성 검사가 수행되지만, 더 복잡한 유효성 검사 규칙을 적용하고 싶을 수도 있습니다.

Laravel MCP는 Laravel의 [유효성 검사 기능](/docs/{{version}}/validation)과 원활하게 통합됩니다. 프롬프트의 `handle` 메서드 내에서 수신된 프롬프트 인수를 검증할 수 있습니다.
...
Laravel [서비스 컨테이너](/docs/{{version}}/container)가 모든 프롬프트를 해석하는 데 사용됩니다. 따라서 프롬프트의 생성자에서 필요한 의존성을 타입-힌트할 수 있습니다. 선언된 의존성은 자동으로 해석되어 프롬프트 인스턴스에 주입됩니다.
...
생성자 주입 외에도, 프롬프트의 `handle` 메서드에서 의존성을 타입-힌트할 수도 있습니다. 메서드가 호출될 때 서비스 컨테이너가 자동으로 의존성을 해석하고 주입합니다.
...
프롬프트 클래스에서 `shouldRegister` 메서드를 구현하여 런타임에 조건부로 프롬프트를 등록할 수 있습니다. 이 메서드를 통해 애플리케이션 상태, 설정 또는 요청 파라미터에 따라 프롬프트의 사용 가능 여부를 결정할 수 있습니다.
...
프롬프트의 `shouldRegister` 메서드가 `false`를 반환하면 사용 가능한 프롬프트 목록에 나타나지 않으며 AI 클라이언트에 의해 호출될 수 없습니다.

<a name="prompt-responses"></a>
### 프롬프트 응답

프롬프트는 단일 `Laravel\Mcp\Response` 또는 `Laravel\Mcp\Response` 인스턴스의 이터러블을 반환할 수 있습니다. 이러한 응답은 AI 클라이언트에 전송될 콘텐츠를 캡슐화합니다.
...
`asAssistant()` 메서드를 사용하여 응답 메시지가 AI 어시스턴트에서 온 것으로 처리되어야 함을 나타낼 수 있으며, 일반 메시지는 사용자 입력으로 처리됩니다.

<a name="resources"></a>
## 리소스

[리소스](https://modelcontextprotocol.io/specification/2025-06-18/server/resources)는 AI 클라이언트가 언어 모델과 상호작용할 때 읽고 컨텍스트로 사용할 수 있는 데이터와 콘텐츠를 서버에서 노출할 수 있게 해줍니다. 문서, 설정 또는 AI 응답에 도움이 되는 모든 데이터와 같은 정적 또는 동적 정보를 공유하는 방법을 제공합니다.
...
기본적으로 리소스의 이름과 제목은 클래스 이름에서 자동으로 생성됩니다. 예를 들어, `WeatherGuidelinesResource`의 이름은 `weather-guidelines`이고 제목은 `Weather Guidelines Resource`가 됩니다. `Name`과 `Title` 어트리뷰트를 사용하여 이 값들을 커스터마이즈할 수 있습니다:
...
리소스 설명은 자동으로 생성되지 않습니다. 항상 `Description` 어트리뷰트를 사용하여 의미 있는 설명을 제공해야 합니다:
...
> [!NOTE]
> 설명은 리소스의 메타데이터에서 중요한 부분으로, AI 모델이 리소스를 언제 어떻게 효과적으로 사용할 수 있는지 이해하는 데 도움을 줍니다.

<a name="resource-templates"></a>
### 리소스 템플릿

[리소스 템플릿](https://modelcontextprotocol.io/specification/2025-06-18/server/resources#resource-templates)은 변수가 포함된 URI 패턴과 일치하는 동적 리소스를 서버에서 노출할 수 있게 해줍니다. 각 리소스에 대해 정적 URI를 정의하는 대신, 템플릿 패턴을 기반으로 여러 URI를 처리하는 단일 리소스를 생성할 수 있습니다.

<a name="creating-resource-templates"></a>
#### 리소스 템플릿 생성

리소스 템플릿을 생성하려면 리소스 클래스에 `HasUriTemplate` 인터페이스를 구현하고 `UriTemplate` 인스턴스를 반환하는 `uriTemplate` 메서드를 정의하세요.
...
리소스가 `HasUriTemplate` 인터페이스를 구현하면 정적 리소스가 아닌 리소스 템플릿으로 등록됩니다. AI 클라이언트는 템플릿 패턴과 일치하는 URI를 사용하여 리소스를 요청할 수 있으며, URI의 변수는 자동으로 추출되어 리소스의 `handle` 메서드에서 사용할 수 있게 됩니다.

<a name="uri-template-syntax"></a>
#### URI 템플릿 구문

URI 템플릿은 중괄호로 감싼 플레이스홀더를 사용하여 URI의 변수 세그먼트를 정의합니다.
...
URI가 리소스 템플릿과 일치하면 추출된 변수가 자동으로 요청에 병합되며 `get` 메서드를 사용하여 접근할 수 있습니다.
...
각 리소스는 고유한 URI로 식별되며, AI 클라이언트가 리소스의 형식을 이해할 수 있도록 관련 MIME 타입을 갖습니다.
...
`Uri` 및 `MimeType` 어트리뷰트를 사용하여 이 값들을 커스터마이즈할 수 있습니다:
...
도구 및 프롬프트와 달리 리소스는 입력 스키마나 인수를 정의할 수 없습니다. 하지만 리소스의 `handle` 메서드 내에서 요청 객체와 상호작용할 수 있습니다.
...
Laravel [서비스 컨테이너](/docs/{{version}}/container)가 모든 리소스를 해석하는 데 사용됩니다. 따라서 리소스의 생성자에서 필요한 의존성을 타입-힌트할 수 있습니다. 선언된 의존성은 자동으로 해석되어 리소스 인스턴스에 주입됩니다.
...
생성자 주입 외에도, 리소스의 `handle` 메서드에서 의존성을 타입-힌트할 수도 있습니다. 메서드가 호출될 때 서비스 컨테이너가 자동으로 의존성을 해석하고 주입합니다.
...
[어노테이션](https://modelcontextprotocol.io/specification/2025-06-18/schema#resourceannotations)을 사용하여 AI 클라이언트에 추가 메타데이터를 제공함으로써 리소스를 향상시킬 수 있습니다. 어노테이션은 어트리뷰트를 통해 리소스에 추가됩니다.
...
| 어노테이션         | 타입          | 설명                                                                        |
| ----------------- | ------------- | --------------------------------------------------------------------------- |
| `#[Audience]`     | Role 또는 배열 | 의도된 대상을 지정합니다 (`Role::User`, `Role::Assistant`, 또는 둘 다).          |
| `#[Priority]`     | float         | 리소스 중요도를 나타내는 0.0에서 1.0 사이의 수치 점수입니다.                      |
| `#[LastModified]` | string        | 리소스가 마지막으로 업데이트된 시점을 나타내는 ISO 8601 타임스탬프입니다.            |

<a name="conditional-resource-registration"></a>
### 조건부 리소스 등록

리소스 클래스에서 `shouldRegister` 메서드를 구현하여 런타임에 조건부로 리소스를 등록할 수 있습니다. 이 메서드를 통해 애플리케이션 상태, 설정 또는 요청 파라미터에 따라 리소스의 사용 가능 여부를 결정할 수 있습니다.
...
리소스의 `shouldRegister` 메서드가 `false`를 반환하면 사용 가능한 리소스 목록에 나타나지 않으며 AI 클라이언트가 접근할 수 없습니다.

<a name="resource-responses"></a>
### 리소스 응답

리소스는 `Laravel\Mcp\Response` 인스턴스를 반환해야 합니다. Response 클래스는 다양한 유형의 응답을 생성하기 위한 여러 편리한 메서드를 제공합니다.

간단한 텍스트 콘텐츠의 경우 `text` 메서드를 사용하세요.
...
blob 콘텐츠를 반환하려면 blob 콘텐츠를 제공하여 `blob` 메서드를 사용하세요.
...
리소스 조회 중 오류가 발생했음을 나타내려면 `error()` 메서드를 사용하세요.
...
Laravel MCP는 특정 MCP 클라이언트 또는 통합에 필요한 [MCP 사양](https://modelcontextprotocol.io/specification/2025-06-18/basic#meta)에 정의된 `_meta` 필드도 지원합니다. 메타데이터는 도구, 리소스, 프롬프트를 포함한 모든 MCP 프리미티브와 그 응답에 적용할 수 있습니다.
...
전체 응답 엔벨로프에 적용되는 결과 수준의 메타데이터의 경우, `Response::make`로 응답을 감싸고 반환된 응답 팩토리 인스턴스에서 `withMeta`를 호출하세요.
...
라우트와 마찬가지로, 미들웨어를 사용하여 웹 MCP 서버를 인증(Authentication)할 수 있습니다. MCP 서버에 인증을 추가하면 사용자가 서버의 모든 기능을 사용하기 전에 인증해야 합니다.

MCP 서버에 대한 접근을 인증하는 방법은 두 가지가 있습니다. [Laravel Sanctum](/docs/{{version}}/sanctum)을 통한 간단한 토큰 기반 인증 또는 `Authorization` HTTP 헤더를 통해 전달되는 모든 토큰을 사용하는 방법입니다. 또는 [Laravel Passport](/docs/{{version}}/passport)를 사용하여 OAuth를 통해 인증할 수 있습니다.

<a name="oauth"></a>
### OAuth 2.1

웹 기반 MCP 서버를 보호하는 가장 강력한 방법은 [Laravel Passport](/docs/{{version}}/passport)를 사용한 OAuth입니다.

OAuth를 통해 MCP 서버를 인증할 때, `routes/ai.php` 파일에서 `Mcp::oauthRoutes` 메서드를 호출하여 필요한 OAuth2 디스커버리 및 클라이언트 등록 라우트를 등록하세요. 그런 다음, `routes/ai.php` 파일의 `Mcp::web` 라우트에 Passport의 `auth:api` 미들웨어를 적용하세요.
...
애플리케이션에서 아직 Laravel Passport를 사용하고 있지 않다면, Passport의 [설치 및 배포 가이드](/docs/{{version}}/passport#installation)를 따라 애플리케이션에 Passport를 추가하세요. 계속 진행하기 전에 `OAuthenticatable` 모델, 새로운 인증 가드, passport 키가 준비되어 있어야 합니다.
...
그런 다음, `Passport::authorizationView` 메서드를 사용하여 Passport가 이 뷰를 사용하도록 지시하세요. 일반적으로 이 메서드는 애플리케이션의 `AppServiceProvider`의 `boot` 메서드에서 호출해야 합니다.
...
> [!NOTE]
> 이 시나리오에서는 OAuth를 기본 인증 가능 모델에 대한 변환 계층으로 단순히 사용하고 있습니다. 스코프와 같은 OAuth의 많은 측면은 무시합니다.

#### 기존 Passport 설치 사용

애플리케이션에서 이미 Laravel Passport를 사용하고 있다면, Laravel MCP는 기존 Passport 설치와 원활하게 작동합니다. 다만 OAuth가 주로 기본 인증 가능 모델에 대한 변환 계층으로 사용되므로 커스텀 스코프는 현재 지원되지 않습니다.

Laravel MCP는 위에서 설명한 `Mcp::oauthRoutes` 메서드를 통해 단일 `mcp:use` 스코프를 추가하고, 알리고, 사용합니다.

#### Passport vs. Sanctum

OAuth 2.1은 Model Context Protocol 사양에 문서화된 인증 메커니즘이며, MCP 클라이언트 중에서 가장 널리 지원됩니다. 이러한 이유로 가능하면 Passport를 사용하는 것을 권장합니다.

애플리케이션에서 이미 [Sanctum](/docs/{{version}}/sanctum)을 사용하고 있다면 Passport를 추가하는 것이 번거로울 수 있습니다. 이 경우, OAuth만 지원하는 MCP 클라이언트를 사용해야 하는 명확하고 필요한 요구사항이 생길 때까지 Passport 없이 Sanctum을 사용하는 것을 권장합니다.

<a name="sanctum"></a>
### Sanctum

[Sanctum](/docs/{{version}}/sanctum)을 사용하여 MCP 서버를 보호하려면, `routes/ai.php` 파일에서 서버에 Sanctum의 인증 미들웨어를 추가하기만 하면 됩니다. 그런 다음, MCP 클라이언트가 성공적인 인증을 위해 `Authorization: Bearer <token>` 헤더를 제공하도록 하세요.
...
애플리케이션이 자체 커스텀 API 토큰을 발급하는 경우, `Mcp::web` 라우트에 원하는 미들웨어를 할당하여 MCP 서버를 인증할 수 있습니다. 커스텀 미들웨어에서 `Authorization` 헤더를 수동으로 검사하여 수신된 MCP 요청을 인증할 수 있습니다.

<a name="authorization"></a>
## 인가

`$request->user()` 메서드를 통해 현재 인증된 사용자에 접근할 수 있으며, 이를 통해 MCP 도구 및 리소스 내에서 [인가(Authorization) 확인](/docs/{{version}}/authorization)을 수행할 수 있습니다.
...
내장된 MCP Inspector 또는 유닛 테스트를 작성하여 MCP 서버를 테스트할 수 있습니다.

<a name="mcp-inspector"></a>
### MCP Inspector

[MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector)는 MCP 서버를 테스트하고 디버깅하기 위한 대화형 도구입니다. 서버에 연결하고, 인증을 확인하고, 도구, 리소스, 프롬프트를 시험해 볼 수 있습니다.
...
이 명령어는 MCP Inspector를 실행하고 MCP 클라이언트에 복사하여 모든 것이 올바르게 설정되었는지 확인할 수 있는 클라이언트 설정을 제공합니다. 웹 서버가 인증 미들웨어로 보호되는 경우, 연결 시 `Authorization` bearer 토큰과 같은 필수 헤더를 포함해야 합니다.

<a name="unit-tests"></a>
### 유닛 테스트

MCP 서버, 도구, 리소스, 프롬프트에 대한 유닛 테스트를 작성할 수 있습니다.
...
프리미티브를 호출하기 전에 `actingAs` 메서드를 체이닝하여 인증된 사용자로 행동할 수도 있습니다.
...
`assertOk` 메서드를 사용하여 응답이 성공적인지 확인할 수 있습니다. 이 메서드는 응답에 오류가 없는지 확인합니다.
...
`assertSentNotification`과 `assertNotificationCount` 메서드를 사용하여 알림이 전송되었는지 확인할 수 있습니다.
...
마지막으로, 원시 응답 콘텐츠를 검사하고 싶다면 `dd` 또는 `dump` 메서드를 사용하여 디버깅 목적으로 응답을 출력할 수 있습니다.
