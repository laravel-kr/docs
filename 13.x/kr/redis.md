...
기본 설정 옵션 외에도, PhpRedis는 다음과 같은 추가 연결 파라미터를 지원합니다: `name`, `persistent`, `persistent_id`, `prefix`, `read_timeout`, `retry_interval`, `max_retries`, `backoff_algorithm`, `backoff_base`, `backoff_cap`, `timeout`, `context`. `config/database.php` 설정 파일의 Redis 서버 설정에 이러한 옵션을 추가할 수 있습니다.
...
Predis 3.4.0 이상에서는 `Retry` 클래스를 통해 내장 재시도 및 백오프 설정을 지원합니다. `retry` 옵션을 사용하여 `NoBackoff`, `EqualBackoff` 또는 `ExponentialBackoff` 전략 중 하나로 설정할 수 있습니다:
...
Redis 연결은 TCP 대신 유닉스 소켓을 사용하도록 설정할 수도 있습니다. 이는 애플리케이션과 동일한 서버에 있는 Redis 인스턴스에 대한 연결의 TCP 오버헤드를 제거하여 성능을 향상시킬 수 있습니다. 유닉스 소켓을 사용하도록 Redis를 설정하려면, `REDIS_HOST` 환경 변수를 Redis 소켓 경로로 설정하고 `REDIS_PORT` 환경 변수를 `0`으로 설정하세요.
