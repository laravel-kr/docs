...
또는, `dd`, `ddHeaders`, `ddBody`, `ddJson`, `ddSession` 메소드를 사용하여 응답에 대한 정보를 덤프한 다음 실행을 중지할 수 있습니다.
...
때때로 애플리케이션이 특정 예외를 발생시키는지 테스트해야 할 수 있습니다. 이를 위해 `Exceptions` 파사드를 통해 예외 핸들러를 "가짜"로 만들 수 있습니다. 예외 핸들러가 가짜로 설정되면, `assertReported` 및 `assertNotReported` 메소드를 사용하여 요청 중에 발생한 예외에 대한 assertion을 만들 수 있습니다.
...
`assertNotReported` 및 `assertNothingReported` 메소드를 사용하여 요청 중에 주어진 예외가 발생하지 않았거나 예외가 전혀 발생하지 않았음을 검증할 수 있습니다.
...
요청을 보내기 전에 `withoutExceptionHandling` 메소드를 호출하여 주어진 요청에 대한 예외 처리를 완전히 비활성화할 수 있습니다.
...
발생한 예외를 검사하고 assertion을 만들고 싶다면 `assertThrows` 메소드의 두 번째 인수로 클로저를 제공할 수 있습니다.
...
`assertDoesntThrow` 메소드를 사용하여 주어진 클로저 내의 코드가 예외를 발생시키지 않는지 검증할 수 있습니다.
...
> [!NOTE]
> `assertJson` 메소드는 응답을 배열로 변환하여 애플리케이션에서 반환된 JSON 응답 내에 주어진 배열이 존재하는지 확인합니다. 따라서 JSON 응답에 다른 속성이 있더라도 주어진 조각이 존재하는 한 이 테스트는 통과합니다.
...
JSON 컬렉션의 모든 항목에 대해 동일한 assertion을 수행하려면 `each` 메서드를 사용할 수 있습니다:
...
응답에 주어진 JSON 구조와 정확히 일치하는 내용이 포함되어 있는지 검증합니다.

```php
$response->assertExactJsonStructure(array $data);
```

이 메소드는 [assertJsonStructure](#assert-json-structure)의 더 엄격한 변형입니다. `assertJsonStructure`와 달리 이 메소드는 응답에 예상된 JSON 구조에 명시적으로 포함되지 않은 키가 포함되어 있으면 실패합니다.
...
주어진 필드들만 유효성 검사 에러가 있는지 검증하려면 `assertOnlyInvalid` 메소드를 사용할 수 있습니다.
