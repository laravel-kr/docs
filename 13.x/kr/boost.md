...
기본적으로 `boost:update` 명령은 애플리케이션 내에 이미 퍼블리시된 기존 Boost 리소스만 업데이트합니다. Boost가 애플리케이션에서 새로 설치된 패키지를 스캔하고 해당하는 가이드라인과 스킬을 퍼블리시할 것인지 제안하도록 하려면 `--discover` 옵션을 사용할 수 있습니다.
