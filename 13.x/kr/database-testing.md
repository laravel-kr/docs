...
또는 `RefreshDatabase` 트레이트를 사용하는 각 테스트 전에 자동으로 데이터베이스를 시딩하도록 Laravel에 지시할 수 있습니다. 기본 테스트 클래스에 `Seed` 속성을 추가하여 이를 수행할 수 있습니다.
...
`Seed` 속성이 있는 경우, `RefreshDatabase` 트레이트를 사용하는 각 테스트 전에 `Database\Seeders\DatabaseSeeder` 클래스가 실행됩니다. 그러나 테스트 클래스에서 `Seeder` 속성을 사용하여 실행할 특정 시더를 지정할 수 있습니다.
