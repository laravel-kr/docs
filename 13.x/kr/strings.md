...
`Str::chopStart` 메서드는 문자열의 시작 부분에 해당 값이 나타나는 경우에만 그 값의 첫 번째 발생을 제거합니다.
...
두 번째 인수로 배열을 전달할 수도 있습니다. 문자열이 배열의 어떤 값으로 시작하면 해당 값이 문자열에서 제거됩니다.
...
`Str::chopEnd` 메서드는 문자열의 끝 부분에 해당 값이 나타나는 경우에만 그 값의 마지막 발생을 제거합니다.
...
두 번째 인수로 배열을 전달할 수도 있습니다. 문자열이 배열의 어떤 값으로 끝나면 해당 값이 문자열에서 제거됩니다.
...
주어진 문자열이 배열의 값 중 어떤 것도 포함하지 않는지 확인하기 위해 값 배열을 전달할 수도 있습니다.
...
`Str::deduplicate` 메서드는 주어진 문자열에서 연속된 문자 인스턴스를 해당 문자의 단일 인스턴스로 교체합니다. 기본적으로 이 메서드는 공백을 중복 제거합니다.
...
문자열을 자를 때 완전한 단어를 유지하려면 `preserveWords` 인수를 활용할 수 있습니다. 이 인수가 `true`이면, 문자열은 가장 가까운 완전한 단어 경계까지 잘립니다.
...
`Str::transliterate` 메소드는 주어진 문자열(String)을 가장 가까운 ASCII 표현으로 변환하려고 시도합니다.
...
`Str::trim` 메소드는 주어진 문자열(String)의 시작과 끝에서 공백(또는 다른 문자)을 제거합니다. PHP의 기본 `trim` 함수와 달리 `Str::trim` 메소드는 유니코드 공백 문자도 제거합니다.
...
`Str::ltrim` 메소드는 주어진 문자열(String)의 시작 부분에서 공백(또는 다른 문자)을 제거합니다. PHP의 기본 `ltrim` 함수와 달리 `Str::ltrim` 메소드는 유니코드 공백 문자도 제거합니다.
...
`Str::rtrim` 메소드는 주어진 문자열(String)의 끝 부분에서 공백(또는 다른 문자)을 제거합니다. PHP의 기본 `rtrim` 함수와 달리 `Str::rtrim` 메소드는 유니코드 공백 문자도 제거합니다.
...
`Str::uuid7` 메소드는 UUID(버전 7)를 생성합니다.
...
순서가 지정된 UUID를 생성하는 데 사용할 `DateTimeInterface`를 선택적 매개변수로 전달할 수 있습니다.
...
`chopStart` 메소드는 주어진 값이 문자열(String)의 시작 부분에 나타나는 경우에만 해당 값의 첫 번째 발생을 제거합니다.
...
`chopEnd` 메소드는 주어진 값이 문자열(String)의 끝 부분에 나타나는 경우에만 해당 값의 마지막 발생을 제거합니다.
...
배열을 전달할 수도 있습니다. 문자열이 배열의 값 중 하나로 끝나면 해당 값이 문자열에서 제거됩니다.
...
`contains` 메소드는 주어진 문자열(String)이 주어진 값을 포함하는지 확인합니다. 기본적으로 이 메소드는 대소문자를 구분합니다.
...
`deduplicate` 메소드는 주어진 문자열(String)에서 연속된 문자 인스턴스를 해당 문자의 단일 인스턴스로 대체합니다. 기본적으로 이 메소드는 공백을 중복 제거합니다.
...
메소드의 두 번째 인수로 전달하여 중복 제거할 다른 문자를 지정할 수 있습니다.
...
문자열을 자를 때 완전한 단어를 유지하려면 `preserveWords` 인자를 사용할 수 있습니다. 이 인자가 `true`일 때 문자열은 가장 가까운 완전한 단어 경계까지 잘립니다.
...
표현식 내에 매칭 그룹을 지정하면 Laravel은 첫 번째 매칭 그룹의 일치 항목을 포함하는 컬렉션을 반환합니다.
...
`toHtmlString` 메소드는 주어진 문자열을 `Illuminate\Support\HtmlString` 인스턴스로 변환하며, Blade 템플릿에서 렌더링될 때 이스케이프되지 않습니다.
...
`transliterate` 메소드는 주어진 문자열을 가장 가까운 ASCII 표현으로 변환하려고 시도합니다.
...
`trim` 메소드는 주어진 문자열을 트리밍합니다. PHP의 기본 `trim` 함수와 달리 Laravel의 `trim` 메소드는 유니코드 공백 문자도 제거합니다.
...
`ltrim` 메소드는 문자열의 왼쪽을 트리밍합니다. PHP의 기본 `ltrim` 함수와 달리 Laravel의 `ltrim` 메소드는 유니코드 공백 문자도 제거합니다.
...
`rtrim` 메소드는 주어진 문자열의 오른쪽을 트리밍합니다. PHP의 기본 `rtrim` 함수와 달리 Laravel의 `rtrim` 메소드는 유니코드 공백 문자도 제거합니다.
...
`wrap` 메소드는 주어진 문자열을 추가 문자열 또는 한 쌍의 문자열로 감쌉니다.
