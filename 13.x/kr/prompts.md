...
`info` 인자를 사용하여 현재 강조된 옵션에 대한 추가 정보를 표시할 수 있습니다. 클로저가 제공되면 현재 강조된 옵션의 값을 받아 문자열 또는 `null`을 반환해야 합니다:
...
`autocomplete` 함수는 가능한 선택지에 대한 인라인 자동 완성을 제공하는 데 사용할 수 있습니다. 사용자가 입력하면 입력과 일치하는 제안이 고스트 텍스트로 나타나며 `Tab` 또는 오른쪽 화살표 키를 눌러 수락할 수 있습니다:
...
사용자 입력에 따라 동적으로 옵션을 생성하기 위해 클로저를 전달할 수도 있습니다. 클로저는 사용자가 문자를 입력할 때마다 호출되며 자동 완성을 위한 옵션 배열을 반환해야 합니다:
...
값이 입력되어야 하는 경우 `required` 인자를 전달할 수 있습니다:
...
추가 유효성 검사 로직을 수행하려면 `validate` 인자에 클로저를 전달할 수 있습니다:
...
클로저는 입력된 값을 받으며 오류 메시지를 반환하거나 유효성 검사가 통과하면 `null`을 반환할 수 있습니다.
...
`task` 함수는 주어진 콜백이 실행되는 동안 레이블이 있는 작업을 스피너와 스크롤 라이브 출력 영역과 함께 표시합니다. 의존성 설치나 배포 스크립트와 같은 장시간 실행 프로세스를 래핑하여 실시간으로 진행 상황을 확인하는 데 이상적입니다:
...
콜백은 작업의 출력 영역에 로그 라인, 상태 메시지 및 스트리밍 텍스트를 표시하는 데 사용할 수 있는 `Logger` 인스턴스를 받습니다.

> [!WARNING]
> `task` 함수는 스피너 애니메이션을 위해 [PCNTL](https://www.php.net/manual/en/book.pcntl.php) PHP 확장이 필요합니다. 이 확장을 사용할 수 없는 경우 작업의 정적 버전이 대신 표시됩니다.

<a name="task-logging"></a>
#### 로그 라인

`line` 메서드는 작업의 스크롤 출력 영역에 단일 로그 라인을 씁니다:
...
`success`, `warning`, `error` 메서드를 사용하여 상태 메시지를 표시할 수 있습니다. 이들은 스크롤 로그 영역 위에 안정적이고 강조된 메시지로 나타납니다:
...
AI 생성 응답과 같이 점진적으로 출력을 생성하는 프로세스의 경우, `partial` 메서드를 사용하여 단어별 또는 청크별로 텍스트를 스트리밍할 수 있습니다. 스트림이 완료되면 `commitPartial`을 호출하여 출력을 확정합니다:
...
기본적으로 작업은 최대 10줄의 스크롤 출력을 표시합니다. `limit` 인자를 통해 이를 커스터마이즈할 수 있습니다:
...
`stream` 함수는 터미널로 스트리밍되는 텍스트를 표시하며, AI 생성 콘텐츠나 점진적으로 도착하는 텍스트를 표시하는 데 이상적입니다:
...
foreach ($words as $word) {
    $stream->append($word . ' ');
    usleep(25_000); // 청크 간 지연 시뮬레이션...
}

$stream->close();
```

`append` 메서드는 스트림에 텍스트를 추가하여 점진적 페이드인 효과로 렌더링합니다. 모든 콘텐츠가 스트리밍되면 `close` 메서드를 호출하여 출력을 확정하고 커서를 복원합니다.
