...
데이터베이스 테이블에서 단일 행을 조회하되, 일치하는 행이 없으면 `Illuminate\Database\RecordNotFoundException`을 던지고 싶다면 `firstOrFail` 메서드를 사용할 수 있습니다. `RecordNotFoundException`이 잡히지 않으면 404 HTTP 응답이 자동으로 클라이언트에게 전송됩니다.
...
`chunkById`와 `lazyById` 메서드는 실행되는 쿼리에 자체적인 "where" 조건을 추가하므로, 일반적으로 클로저 내에서 자신의 조건을 [논리적으로 그룹화](#logical-grouping)해야 합니다.
...
`lazy` 메서드는 청크로 쿼리를 실행한다는 점에서 [청크 메서드](#chunking-results)와 유사하게 작동합니다. 그러나 각 청크를 콜백에 전달하는 대신 `lazy()` 메서드는 [LazyCollection](/docs/{{version}}/collections#lazy-collections)을 반환하여 결과를 단일 스트림으로 상호작용할 수 있게 해줍니다.
...
> [!WARNING]
> MySQL과 MariaDB는 문자열-숫자 비교에서 문자열을 정수로 자동 형변환합니다. 이 과정에서 숫자가 아닌 문자열은 `0`으로 변환되어 예기치 않은 결과가 발생할 수 있습니다. 예를 들어, 테이블에 `secret` 컬럼의 값이 `aaa`인 행이 있고 `User::where('secret', 0)`을 실행하면 해당 행이 반환됩니다. 이를 방지하려면 쿼리에서 사용하기 전에 모든 값이 적절한 타입으로 형변환되었는지 확인하세요.
...
Laravel은 JSON 컬럼 타입을 지원하는 데이터베이스에서 JSON 컬럼 타입 쿼리도 지원합니다. 현재 MariaDB 10.3+, MySQL 8.0+, PostgreSQL 12.0+, SQL Server 2017+, SQLite 3.39.0+가 포함됩니다. JSON 컬럼을 쿼리하려면 `->` 연산자를 사용하세요.
...
`whereLike` 메서드를 사용하면 패턴 매칭을 위한 "LIKE" 절을 쿼리에 추가할 수 있습니다. 이 메서드들은 대소문자 구분을 토글할 수 있는 기능과 함께 데이터베이스에 구애받지 않는 방식으로 문자열 매칭 쿼리를 수행할 수 있게 해줍니다. 기본적으로 문자열 매칭은 대소문자를 구분하지 않습니다.
...
`wherePast`와 `whereFuture` 메서드는 컬럼의 값이 과거인지 미래인지 확인하는 데 사용할 수 있습니다.
...
`whereNowOrPast`와 `whereNowOrFuture` 메서드는 현재 날짜와 시간을 포함하여 컬럼의 값이 과거인지 미래인지 확인하는 데 사용할 수 있습니다.
...
`whereToday`, `whereBeforeToday`, `whereAfterToday` 메서드는 각각 컬럼의 값이 오늘인지, 오늘 이전인지, 오늘 이후인지 확인하는 데 사용할 수 있습니다.
...
마찬가지로 `whereTodayOrBefore`와 `whereTodayOrAfter` 메서드는 오늘 날짜를 포함하여 컬럼의 값이 오늘 이전인지 오늘 이후인지 확인하는 데 사용할 수 있습니다.
...
`whereFullText`와 `orWhereFullText` 메서드는 [전문 인덱스](/docs/{{version}}/migrations#available-index-types)가 있는 컬럼에 대해 전문 검색 "where" 절을 쿼리에 추가하는 데 사용할 수 있습니다. 이 메서드들은 Laravel에 의해 기본 데이터베이스 시스템에 적합한 SQL로 변환됩니다. 예를 들어, MariaDB나 MySQL을 사용하는 애플리케이션의 경우 `MATCH AGAINST` 절이 생성됩니다.
...
> [!NOTE]
> 벡터 유사도 절은 현재 `pgvector` 확장을 사용하는 PostgreSQL 연결에서만 지원됩니다. 벡터 컬럼과 인덱스를 정의하는 방법에 대한 정보는 [마이그레이션 문서](/docs/{{version}}/migrations#available-column-types)를 참조하세요.

`whereVectorSimilarTo` 메서드는 주어진 벡터에 대한 코사인 유사도를 기준으로 결과를 필터링하고 관련성 순으로 정렬합니다. `minSimilarity` 임계값은 `0.0`에서 `1.0` 사이의 값이어야 하며, `1.0`은 동일함을 의미합니다.
...
기본적으로 `whereVectorSimilarTo`는 결과를 거리순(가장 유사한 것 먼저)으로 정렬합니다. `order` 인수에 `false`를 전달하여 이 정렬을 비활성화할 수 있습니다.
...
더 세밀한 제어가 필요하면 `selectVectorDistance`, `whereVectorDistanceLessThan`, `orderByVectorDistance` 메서드를 독립적으로 사용할 수 있습니다.
...
PostgreSQL을 사용할 때, `vector` 컬럼을 생성하기 전에 `pgvector` 확장이 로드되어야 합니다.
...
> [!WARNING]
> SQL Server를 제외한 모든 데이터베이스는 `upsert` 메서드의 두 번째 인수에 있는 컬럼에 "primary" 또는 "unique" 인덱스가 있어야 합니다. 또한 MariaDB와 MySQL 데이터베이스 드라이버는 `upsert` 메서드의 두 번째 인수를 무시하고 항상 테이블의 "primary" 및 "unique" 인덱스를 사용하여 기존 레코드를 감지합니다.
...
`updateOrInsert` 메서드에 클로저를 제공하여 일치하는 레코드의 존재 여부에 따라 데이터베이스에 업데이트하거나 삽입할 속성을 커스터마이즈할 수 있습니다.
...
JSON 컬럼을 업데이트할 때 `->` 구문을 사용하여 JSON 객체의 적절한 키를 업데이트해야 합니다. 이 작업은 MariaDB 10.3+, MySQL 5.7+, PostgreSQL 9.5+에서 지원됩니다.
...
필수는 아니지만 비관적 잠금을 [트랜잭션](/docs/{{version}}/database#database-transactions) 내에서 래핑하는 것이 좋습니다. 이렇게 하면 조회된 데이터가 전체 작업이 완료될 때까지 데이터베이스에서 변경되지 않습니다. 실패의 경우 트랜잭션은 모든 변경 사항을 롤백하고 자동으로 잠금을 해제합니다.
...
애플리케이션 전체에서 반복되는 쿼리 로직이 있는 경우 쿼리 빌더의 `tap`과 `pipe` 메서드를 사용하여 로직을 재사용 가능한 객체로 추출할 수 있습니다. 애플리케이션에 다음 두 가지 다른 쿼리가 있다고 가정해 보겠습니다.
...
쿼리 간에 공통인 목적지 필터링을 재사용 가능한 객체로 추출하고 싶을 수 있습니다.
...
애플리케이션 전체에서 사용되는 공유 [페이지네이션](/docs/{{version}}/pagination) 로직을 포함하는 다음 쿼리 객체를 고려하세요. 쿼리 조건을 쿼리에 적용하는 `DestinationFilter`와 달리 `Paginate` 객체는 쿼리를 실행하고 페이지네이터 인스턴스를 반환합니다.
...
쿼리 빌더의 `pipe` 메서드를 사용하여 이 객체를 활용하여 공유 페이지네이션 로직을 적용할 수 있습니다.
