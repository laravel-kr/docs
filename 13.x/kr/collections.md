...
`after` 메서드는 주어진 아이템 다음의 아이템을 반환합니다. 주어진 아이템을 찾을 수 없거나 마지막 아이템인 경우 `null`이 반환됩니다:
...
이 메서드는 "느슨한(loose)" 비교를 사용하여 주어진 아이템을 검색합니다. 즉, 정수 값을 포함하는 문자열은 같은 값의 정수와 동일하게 간주됩니다. "엄격한(strict)" 비교를 사용하려면 메서드에 `strict` 인수를 제공할 수 있습니다:
...
`before` 메서드는 [after](#method-after) 메서드의 반대입니다. 주어진 아이템 이전의 아이템을 반환합니다. 주어진 아이템을 찾을 수 없거나 첫 번째 아이템인 경우 `null`이 반환됩니다:
...
이 메서드는 [Bootstrap](https://getbootstrap.com/docs/5.3/layout/grid/)과 같은 그리드 시스템을 사용할 때 [뷰](/docs/{{version}}/views)에서 특히 유용합니다. 예를 들어, 그리드에 표시하려는 [Eloquent](/docs/{{version}}/eloquent) 모델 컬렉션이 있다고 가정해 보세요:
...
> [!WARNING]
> 대부분의 다른 컬렉션 메서드와 달리, `forget`은 새로운 수정된 컬렉션을 반환하지 않습니다. 호출된 컬렉션을 수정하고 반환합니다.
...
정적 `fromJson` 메서드는 PHP의 `json_decode` 함수를 사용하여 주어진 JSON 문자열을 디코딩하여 새 컬렉션 인스턴스를 생성합니다:
...
`intersectUsing` 메서드는 사용자 정의 콜백을 사용하여 값을 비교하면서, 주어진 배열 또는 컬렉션에 존재하지 않는 값을 원래 컬렉션에서 제거합니다. 결과 컬렉션은 원래 컬렉션의 키를 유지합니다:
...
`intersectAssocUsing` 메서드는 원래 컬렉션을 다른 컬렉션 또는 배열과 비교하여 키와 값 모두에 대해 사용자 정의 비교 콜백을 사용하여 동등성을 결정하면서 양쪽에 존재하는 키/값 쌍을 반환합니다:
...
`multiply` 메서드는 컬렉션의 모든 아이템을 지정된 횟수만큼 복사하여 생성합니다:
...
여러 속성으로 컬렉션을 정렬하려면 `sortBy` 메서드에 정렬 작업 배열을 전달할 수 있습니다. 각 정렬 작업은 정렬하려는 속성과 원하는 정렬 방향으로 구성된 배열이어야 합니다:
...
`throttle` 메서드는 각 값이 지정된 초 후에 반환되도록 lazy 컬렉션을 조절합니다. 이 메서드는 들어오는 요청에 속도 제한을 두는 외부 API와 상호작용할 때 특히 유용합니다:
