...
`Laravel\Ai\Enums\Lab` 열거형을 사용하면 일반 문자열 대신 코드 전체에서 프로바이더를 참조할 수 있습니다:
...
생성된 미들웨어는 애플리케이션의 `app/Ai/Middleware` 디렉토리에 배치됩니다. 에이전트에 미들웨어를 추가하려면 `HasMiddleware` 인터페이스를 구현하고 미들웨어 클래스 배열을 반환하는 `middleware` 메서드를 정의합니다:
...
에이전트가 프로바이더별 옵션(예: OpenAI의 추론 노력도 또는 패널티 설정)을 전달해야 하는 경우, `HasProviderOptions` 계약을 구현하고 `providerOptions` 메서드를 정의합니다:
...
`providerOptions` 메서드는 현재 사용 중인 프로바이더(`Lab` 열거형 또는 문자열)를 받아, 프로바이더별로 다른 옵션을 반환할 수 있습니다. 이는 [페일오버](#failover)를 사용할 때 특히 유용합니다. 각 대체 프로바이더가 고유한 설정을 받을 수 있기 때문입니다.
