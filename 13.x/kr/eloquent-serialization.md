...
때로는 비밀번호와 같은 속성을 모델의 배열이나 JSON 표현에서 제외하고 싶을 수 있습니다. 이를 위해 모델에 `Hidden` 속성을 사용하세요. `Hidden` 속성에 나열된 속성은 모델의 직렬화된 표현에 포함되지 않습니다.
...
접근자가 항상 모델의 배열 및 JSON 표현에 추가되도록 하려면 모델에 `Appends` 속성을 사용할 수 있습니다. 접근자의 PHP 메서드가 "카멜 케이스(camel case)"로 정의되더라도 속성 이름은 일반적으로 "스네이크 케이스(snake case)" 직렬화 표현을 사용하여 참조됩니다.
...
`Appends` 목록에 속성이 추가되면 모델의 배열 및 JSON 표현 모두에 포함됩니다. `Appends` 속성의 속성도 모델에 구성된 `visible` 및 `hidden` 설정을 따릅니다.
