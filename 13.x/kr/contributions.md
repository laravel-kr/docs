...
**모든** 버그 수정은 버그 수정을 지원하는 최신 버전(현재 `13.x`)으로 보내야 합니다. 버그 수정은 다가오는 릴리스에만 존재하는 기능을 수정하는 경우가 아니라면 **절대로** `master` 브랜치로 보내서는 안 됩니다.
...
Laravel에 제출되는 모든 풀 리퀘스트에 감사드립니다. 그러나 사려 깊은 인간의 검토와 고려 없이 주로 AI로 생성된 기여는 허용되지 않습니다.

AI 도구를 사용하여 기여를 도와받기로 선택한 경우, 결과물 코드는 제출 전에 반드시 직접 철저히 검토, 테스트 및 이해해야 합니다.

**전적으로 AI로 생성된 이슈나 풀 리퀘스트를 대량으로 여는 것은 용납되지 않습니다.** 그러한 풀 리퀘스트는 검토 없이 닫히며, 기여한 사용자는 저장소에서 차단될 수 있습니다.

기여자들은 기존 코드베이스를 숙지하고, 커뮤니티와 소통하며, 해결하려는 문제에 대한 자신의 이해와 신중한 고려를 반영한 풀 리퀘스트를 제출하는 것을 권장합니다.
