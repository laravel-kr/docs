...
Scout는 또한 대규모에서 오타 허용, 패싯 필터링 또는 지역 검색과 같은 기능이 필요할 때 [Algolia](https://www.algolia.com/), [Meilisearch](https://www.meilisearch.com), [Typesense](https://typesense.org) 드라이버도 포함합니다. 로컬 개발을 위한 "컬렉션" 드라이버도 사용 가능하며, [커스텀 엔진](#custom-engines)을 자유롭게 작성할 수도 있습니다.
...
Scout를 사용하면 검색 쿼리에 "where" 절을 추가할 수 있습니다. 예를 들어, 기본적인 동등 검사는 소유자 ID로 검색 쿼리의 범위를 지정하는 데 유용합니다:
...
`=`, `!=`, `<`, `>`, `>=`, `<=` 비교 연산자를 사용하여 더 고급 쿼리를 구성할 수도 있습니다:
