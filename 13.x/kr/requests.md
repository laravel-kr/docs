...
요청이 특히 Markdown을 선호하는지 또는 다른 콘텐츠 타입 중에서 Markdown을 수용하는지 확인해야 하는 경우, 예를 들어 AI 에이전트나 Markdown 응답을 소비하는 다른 클라이언트를 서빙할 때, `wantsMarkdown`과 `acceptsMarkdown` 메소드를 사용할 수 있습니다.
...
기간(duration)을 포함하는 입력값은 `interval` 메소드를 사용하여 `CarbonInterval` 인스턴스로 조회할 수 있습니다. 요청에 주어진 이름의 입력값이 없으면 `null`이 반환됩니다.

```php
$duration = $request->interval('duration');
```

입력값이 숫자인 경우, 두 번째 인수로 단위를 제공할 수 있습니다. 단위는 `second`, `minute`, `day`와 같은 문자열이거나 `Carbon\Unit` 열거형 인스턴스일 수 있습니다.
...
입력값이 있지만 형식이 잘못된 경우 `InvalidArgumentException`이 발생합니다. 따라서 `interval` 메소드를 호출하기 전에 입력을 유효성 검사하는 것이 좋습니다.
