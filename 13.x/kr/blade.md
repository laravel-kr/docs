...
동일한 네임스페이스 내에 여러 클래스가 있는 경우 해당 클래스들의 import를 그룹화할 수 있습니다.

```blade
@use('App\Models\{Flight, Airport}')
```

`@use` 지시어는 import 경로에 `function` 또는 `const` 수정자를 접두사로 붙여 PHP 함수와 상수를 가져오는 것도 지원합니다.
...
function과 const 수정자 모두에서 그룹화된 import도 지원되어, 단일 지시어에서 동일한 네임스페이스의 여러 심볼을 가져올 수 있습니다.
...
때때로 컴포넌트는 컴포넌트 그룹의 일부이며 관련 컴포넌트를 단일 디렉토리 내에 그룹화하려 할 수 있습니다. 예를 들어, 다음 클래스 구조를 가진 "card" 컴포넌트를 상상해 보세요.
...
`only` 메서드를 사용하여 주어진 키를 가진 속성만 검색할 수 있습니다.

```blade
{{ $attributes->only(['class']) }}
```

`except` 메서드를 사용하여 주어진 키를 가진 속성을 제외한 모든 속성을 검색할 수 있습니다.
