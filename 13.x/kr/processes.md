...
`timeout` 및 `idleTimeout` 메서드는 `CarbonInterval` 인스턴스도 허용합니다:
...
비동기 프로세스가 실행되는 동안 `ensureNotTimedOut` 메서드를 사용하여 프로세스가 타임아웃되지 않았는지 확인할 수 있습니다. 이 메서드는 프로세스가 타임아웃된 경우 [타임아웃 예외](#timeouts)를 던집니다.
