...
[Laravel Pint](https://github.com/laravel/pint)는 미니멀리스트를 위한 독자적인 PHP 코드 스타일 수정 도구입니다. Pint는 [PHP CS Fixer](https://github.com/FriendsOfPHP/PHP-CS-Fixer)를 기반으로 구축되었으며, 코드 스타일을 깔끔하고 일관성 있게 유지할 수 있도록 도와줍니다.
...
PHP CS Fixer 규칙 외에도 Pint는 `Pint/` 접두사가 붙은 커스텀 규칙을 제공합니다. 이 규칙들은 기본적으로 활성화되어 있지 않지만, `pint.json` 파일에서 활성화할 수 있습니다.

<a name="phpdoc-type-annotations-only"></a>
##### `Pint/phpdoc_type_annotations_only`

이 규칙은 코드에서 모든 주석과 독블록 설명을 제거하고, `@param`, `@return`, `@var`, `@phpstan-type` 등과 같은 `@` 어노테이션이 포함된 줄만 유지합니다.
...
`@` 어노테이션이 없는 한 줄 주석과 블록 주석은 완전히 제거됩니다. 특정 주석을 유지하고 싶다면 `@note`, `@warning`, 또는 `@todo` 접두사를 붙일 수 있습니다.
...
> [!NOTE]
> 이 규칙은 `config` 디렉토리의 파일을 자동으로 건너뜁니다. 설정 파일은 일반적으로 문서화를 위해 주석에 의존하기 때문입니다.
