...
> [!NOTE]
> 내부적으로 Laravel은 PHP의 출력 버퍼링 기능을 활용합니다. 위 예제에서 볼 수 있듯이, 버퍼링된 콘텐츠를 클라이언트에 전달하려면 `ob_flush`와 `flush` 함수를 사용해야 합니다.
...
이 이벤트 스트림은 애플리케이션의 프론트엔드에서 [EventSource](https://developer.mozilla.org/en-US/docs/Web/API/EventSource) 객체를 통해 사용할 수 있습니다. 스트림이 완료되면 `eventStream` 메서드는 자동으로 이벤트 스트림에 `</stream>` 업데이트를 전송합니다:
