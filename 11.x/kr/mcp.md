...
로컬 서버는 Artisan 명령어로 실행되며, 개발, 테스트 또는 로컬 AI 어시스턴트 통합에 적합합니다. `local` 메서드를 사용하여 로컬 서버를 등록하세요.
...
등록이 완료되면 일반적으로 `mcp:start`를 직접 수동으로 실행할 필요가 없습니다. 대신 MCP 클라이언트(AI 에이전트)가 서버를 시작하도록 구성하세요. `mcp:start` 명령어는 클라이언트가 호출하도록 설계되었으며, 필요에 따라 서버의 시작과 중지를 처리합니다:
...
도구는 AI 클라이언트로부터 받는 인수를 지정하기 위해 입력 스키마를 정의할 수 있습니다. Laravel의 `Illuminate\JsonSchema\JsonSchema` 빌더를 사용하여 도구의 입력 요구사항을 정의하세요.
...
라우트에 대해 하듯이 미들웨어를 사용하여 웹 MCP 서버를 인증(Authentication)할 수 있습니다. 이렇게 하면 사용자가 서버의 모든 기능을 사용하기 전에 인증해야 합니다.

MCP 서버에 대한 접근을 인증하는 방법은 두 가지가 있습니다. [Laravel Sanctum](/docs/{{version}}/sanctum)을 통한 간단한 토큰 기반 인증 또는 `Authorization` HTTP 헤더를 통해 전달되는 기타 임의의 API 토큰을 사용하는 방법입니다. 또는 [Laravel Passport](/docs/{{version}}/passport)를 사용하여 OAuth를 통해 인증할 수 있습니다.

<a name="oauth"></a>
### OAuth 2.1

웹 기반 MCP 서버를 보호하는 가장 강력한 방법은 [Laravel Passport](/docs/{{version}}/passport)를 통한 OAuth입니다.

OAuth를 통해 MCP 서버를 인증할 때는 `routes/ai.php` 파일에서 `Mcp::oauthRoutes` 메서드를 호출하여 필요한 OAuth2 디스커버리 및 클라이언트 등록 라우트를 등록하세요. 그런 다음, `routes/ai.php` 파일의 `Mcp::web` 라우트에 Passport의 `auth:api` 미들웨어를 적용하세요.
...
애플리케이션에서 아직 Laravel Passport를 사용하고 있지 않다면, 먼저 Passport의 [설치 및 배포 단계](/docs/{{version}}/passport#installation)를 따르세요. 계속 진행하기 전에 `OAuthenticatable` 모델, 새로운 인증 가드, passport 키가 준비되어 있어야 합니다.
...
등록된 모든 서버에 대해 Inspector를 실행할 수 있습니다 (예: "weather"라는 이름의 로컬 서버):
