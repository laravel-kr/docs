...
`auth` 미들웨어가 인증되지 않은 사용자를 감지하면, 사용자를 `login` [이름이 지정된 라우트](/docs/{{version}}/routing#named-routes)로 리디렉션합니다. 애플리케이션의 `bootstrap/app.php` 파일의 `redirectGuestsTo` 메서드를 사용하여 이 동작을 수정할 수 있습니다.
