...
**모든** 버그 수정은 버그 수정을 지원하는 최신 버전(현재 `11.x`)으로 보내야 합니다. 버그 수정은 다가오는 릴리스에만 존재하는 기능을 수정하는 경우가 아니라면 **절대로** `master` 브랜치로 보내서는 안 됩니다.

현재 릴리스와 **완전히 하위 호환되는** **마이너** 기능은 최신 안정 브랜치(현재 `11.x`)로 보낼 수 있습니다.
