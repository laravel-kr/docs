...
> [!NOTE]
> 기능 클래스는 [컨테이너](/docs/{{version}}/container)를 통해 해결되므로, 필요할 때 기능 클래스의 생성자에 의존성을 주입할 수 있습니다.
...
때때로 주어진 기능의 저장된 값을 검색하기 전에 일부 인메모리 확인을 수행하는 것이 유용할 수 있습니다. 기능 플래그 뒤에서 새로운 API를 개발하고 있고 스토리지에서 해결된 기능 값을 잃지 않고 새로운 API를 비활성화할 수 있는 기능을 원한다고 상상해 보세요. 새로운 API에서 버그를 발견하면 내부 팀 멤버를 제외한 모든 사람에 대해 쉽게 비활성화하고, 버그를 수정한 다음, 이전에 기능에 액세스할 수 있었던 사용자에게 새로운 API를 다시 활성화할 수 있습니다.

[클래스 기반 기능](#class-based-features)의 `before` 메서드로 이를 달성할 수 있습니다. `before` 메서드가 있으면 스토리지에서 값을 검색하기 전에 항상 인메모리에서 실행됩니다. 메서드에서 `null`이 아닌 값이 반환되면 요청 기간 동안 기능의 저장된 값 대신 사용됩니다.
...
{
    /**
     * 저장된 값이 검색되기 전에 항상 인메모리 확인을 실행합니다.
     */
...
이 기능을 사용하여 이전에 기능 플래그 뒤에 있던 기능의 전역 출시를 예약할 수도 있습니다.
...
> [!NOTE]
> 리치 값을 사용할 때 기능이 `false` 이외의 값을 가질 때 "활성"으로 간주된다는 것을 알아야 합니다.
...
> [!NOTE]
> 이것은 Pennant의 스토리지 드라이버에 의해 저장된 해결된 기능 값만 업데이트합니다. 애플리케이션에서 기능 정의도 업데이트해야 합니다.
...
드라이버가 타사 기능 플래그 플랫폼에 대한 래퍼인 경우 Pennant의 `Feature::define` 메서드를 사용하는 대신 플랫폼에서 기능을 정의할 가능성이 높습니다. 그런 경우 커스텀 드라이버는 `Laravel\Pennant\Contracts\DefinesFeaturesExternally` 인터페이스도 구현해야 합니다.
...
이 이벤트는 [기능이 확인](#checking-features)될 때마다 디스패치됩니다. 이 이벤트는 애플리케이션 전체에서 기능 플래그 사용에 대한 메트릭을 만들고 추적하는 데 유용할 수 있습니다.

### `Laravel\Pennant\Events\FeatureResolved`

이 이벤트는 특정 스코프에 대해 기능의 값이 처음 해결될 때 디스패치됩니다.

### `Laravel\Pennant\Events\UnknownFeatureResolved`

이 이벤트는 특정 스코프에 대해 알 수 없는 기능이 처음 해결될 때 디스패치됩니다. 이 이벤트를 수신하면 기능 플래그를 제거하려고 했지만 애플리케이션 전체에 실수로 참조를 남긴 경우 유용할 수 있습니다.
...
이 상황은 우아하게 처리되며 기능은 `false`를 반환합니다. 그러나 이 기능의 기본 우아한 동작을 선택 해제하려면 애플리케이션의 `AppServiceProvider`의 `boot` 메서드에서 이 이벤트에 대한 리스너를 등록할 수 있습니다.
...
이 이벤트는 스코프에 대한 기능을 업데이트할 때 디스패치되며, 일반적으로 `activate` 또는 `deactivate`를 호출하여 발생합니다.

### `Laravel\Pennant\Events\FeatureUpdatedForAllScopes`

이 이벤트는 모든 스코프에 대한 기능을 업데이트할 때 디스패치되며, 일반적으로 `activateForEveryone` 또는 `deactivateForEveryone`을 호출하여 발생합니다.

### `Laravel\Pennant\Events\FeatureDeleted`

이 이벤트는 스코프에 대한 기능을 삭제할 때 디스패치되며, 일반적으로 `forget`을 호출하여 발생합니다.

### `Laravel\Pennant\Events\FeaturesPurged`

이 이벤트는 특정 기능을 삭제할 때 디스패치됩니다.

### `Laravel\Pennant\Events\AllFeaturesPurged`

이 이벤트는 모든 기능을 삭제할 때 디스패치됩니다.
