...
[FrankenPHP](https://frankenphp.dev)는 Go로 작성된 PHP 애플리케이션 서버로, 조기 힌트(early hints), Brotli, Zstandard 압축과 같은 최신 웹 기능을 지원합니다. Octane을 설치하고 FrankenPHP를 서버로 선택하면, Octane이 자동으로 FrankenPHP 바이너리를 다운로드하고 설치합니다.
...
> [!NOTE]
> 자체 서버 설정을 관리할 준비가 되지 않았거나 강력한 Laravel Octane 애플리케이션을 실행하는 데 필요한 모든 다양한 서비스를 설정하는 것이 익숙하지 않다면, 완전 관리형 Laravel Octane 지원을 제공하는 [Laravel Cloud](https://cloud.laravel.com)를 확인해 보세요.
