...
[Mockery](https://github.com/padraic/mockery) 모킹 인스턴스를 반환하는 `expects` 메서드를 사용하여 `Cache` 파사드 호출을 모킹할 수 있습니다. 파사드는 실제로 Laravel [서비스 컨테이너(Service Container)](/docs/{{version}}/container)에 의해 리졸브되고 관리되기 때문에, 일반적인 정적 클래스보다 훨씬 더 높은 테스트 가능성을 가집니다. 예를 들어, `Cache` 파사드의 `get` 메서드 호출을 모킹해 보겠습니다.
