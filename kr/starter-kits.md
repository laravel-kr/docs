...
[React](#react) 또는 [Vue](#vue) 스타터 킷을 사용하는 경우, 프론트엔드 코드에서 비활성화된 기능의 라우트에 대한 참조도 제거해야 합니다. 예를 들어, 이메일 인증을 비활성화하면 Vue 또는 React 컴포넌트에서 `verification` 라우트의 import 및 참조를 제거해야 합니다. 이러한 스타터 킷은 타입 안전 라우팅을 위해 Wayfinder를 사용하며, 빌드 시 라우트 정의를 생성하기 때문입니다. 더 이상 존재하지 않는 라우트를 참조하면 애플리케이션 빌드가 실패합니다.
